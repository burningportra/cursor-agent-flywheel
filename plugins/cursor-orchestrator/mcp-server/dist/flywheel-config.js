...
export function loadFlywheelConfigWithWarnings(cwd) {
    ...
    if (typeof graderNode === 'object' && graderNode !== null) {
        const g = graderNode;
        const model = typeof g.model === 'string' && g.model.trim() !== '' ? g.model.trim() : undefined;
        if (model)
            grader = { model };
    }
    let impl_tick;
    ...
    if (typeof tickNode === 'object' && tickNode !== null) {
        const t = tickNode;
        const interval_seconds = typeof t.interval_seconds === 'number' && t.interval_seconds >= 60
            ? Math.floor(t.interval_seconds)
            : undefined;
        const review_model = typeof t.review_model === 'string' && t.review_model.trim()
            ? t.review_model.trim()
            : undefined;
