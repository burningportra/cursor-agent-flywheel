 ...
 */
export function beadsToSimulated(beads, depMap) {
    return beads.map((b) => ({
        id: b.id,
        title: b.title,
 ...
 */
export function computeExecutionOrder(beads) {
    if (beads.length === 0)
        return [];
    ...
    if (order.length !== beads.length) {
        ...
        const remaining = beads
            ...
            .map((b) => b.id);
        throw new Error(`Cycle detected — ${remaining.length} bead(s) could not be ordered: ${remaining.join(", ")}`);
    }
    return order;
}
// ─── Parallel Groups ───────────────────────────────────────────
 ...
 */
export function computeParallelGroups(beads) {
    if (beads.length === 0)
        return [];
    const idSet = new Set(beads.map((b) => b.id));
 ...
 */
export function detectFileConflicts(beads, parallelGroups) {
    const beadMap = new Map(beads.map((b) => [b.id, b]));
    const conflicts = [];
 ...
 */
export function detectMissingFiles(beads, repoFiles) {
    const missing = [];
 ...
 */
export function simulateExecutionPaths(beads, repoFiles) {
    const warnings = [];
    ...
    if (fileConflicts.length > 0) {
        warnings.push(`${fileConflicts.length} file conflict(s) between parallel beads`);
    }
    if (missingFiles.length > 0) {
        warnings.push(`${missingFiles.length} file reference(s) to non-existent files`);
    }
 ...
 */
export function formatSimulationReport(result) {
    const lines = [];
    if (result.valid) {
    ...
    if (result.fileConflicts.length > 0) {
        ...
        lines.push("These files are modified by multiple beads in the same parallel group:");
    ...
    if (result.parallelGroups.length > 0) {
        ...
        for (let i = 0; i < result.parallelGroups.length; i++) {
            ...
            lines.push(`- **Level ${i}** (${group.length} bead${group.length !== 1 ? "s" : ""}): ${group.join(", ")}`);
        }
    }
    return lines.join("\n");
}
 ...
 */
const FILE_PATH_RE = /[a-zA-Z0-9_\-./]+(?:\.ts|\.js|\.tsx|\.jsx|\.json|\.md|\.py|\.rs|\.go|\.yaml|\.yml|\.toml)/g;
/**
 ...
 */
function extractFromFilesSection(text) {
 ...
 */
function extractFromProse(text, alreadyCaptured) {
    const result = new Set();
 ...
 */
export function computeHotspotMatrix(beads) {
    ...
    const fileBeadIds = new Map();
    const fileHasFilesSection = new Map();
    for (const bead of sorted) {
        ...
        for (const file of filesSectionPaths) {
            if (!fileBeadIds.has(file))
                fileBeadIds.set(file, new Set());
    ...
    for (const [file, beadIdSet] of fileBeadIds) {
        ...
        const provenance = hasFilesSection ? 'files-section' : 'prose';
        rows.push({
            file,
    ...
    const recommendation = hasMedOrHigh
        ? 'coordinator-serial'
        : 'swarm';
    // Step 6: apply bounds
    ...
    if (summaryOnly) {
        ...
        finalRows = [...rows]
            .sort((a, b) => {
            if (b.contentionCount !== a.contentionCount)
                return b.contentionCount - a.contentionCount;
...
//# sourceMappingURL=plan-simulation.js.map