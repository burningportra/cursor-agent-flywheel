...
export const DEFAULT_CURSOR_DUEL_MODELS = {
    wizard_a: "opus-4.6",
...
export function useNtmDuelBackend() {
...
function configOverride(cwd) {
    ...
    const out = {};
    if (d.wizard_a)
        out.wizard_a = d.wizard_a;
    if (d.wizard_b)
        out.wizard_b = d.wizard_b;
    if (d.wizard_c)
        out.wizard_c = d.wizard_c;
    if (d.synthesis)
        out.synthesis = d.synthesis;
    return Object.keys(out).length > 0 ? out : undefined;
}
export function getCursorDuelModels(cwd) {
    const fromConfig = configOverride(cwd) ?? {};
    return {
        wizard_a: envOverride("wizard_a") ??
            fromConfig.wizard_a ??
            DEFAULT_CURSOR_DUEL_MODELS.wizard_a,
        wizard_b: envOverride("wizard_b") ??
            fromConfig.wizard_b ??
            DEFAULT_CURSOR_DUEL_MODELS.wizard_b,
        wizard_c: envOverride("wizard_c") ??
            fromConfig.wizard_c ??
            DEFAULT_CURSOR_DUEL_MODELS.wizard_c,
        synthesis: envOverride("synthesis") ??
            fromConfig.synthesis ??
            DEFAULT_CURSOR_DUEL_MODELS.synthesis,
    };
}
export function formatCursorDuelModelTable(models) {
    return [
...
export function resolveDuelModelsConfirm(cwd, input) {
...
export function buildDuelModelsGate(cwd) {
    ...
    return {
        ...
        options: [
            ...
            },
            {
                id: "3",
                label: "Custom models",
                detail: 'Reply: wizard_a=<slug> wizard_b=<slug> wizard_c=<slug> (optional third)',
            },
            {
                id: "4",
...
function ideateDeliverable(mode, outputPath, wizardName) {
    if (mode === "ideas") {
...
function buildWizardTasks(slot, model, mode, focus, top, outputPath, profileSummary) {
...
export function buildCursorDuelCoordinatorPlaybook(wizards, outputPath, mode) {
...
export function buildCursorDuelRun(opts) {
    ...
    const profileSummary = opts.profileSummary
        ...
        : "";
    const wizards = activeWizards(models).map(({ slot, model }) => buildWizardTasks(slot, model, mode, focus, top, outputPath, profileSummary));
...
Mode: ${mode} | Focus: ${focus}`;
    ...
    const coordinatorPlaybook = buildCursorDuelCoordinatorPlaybook(wizards, outputPath, mode);
    return {
...
export function defaultTopForMode(mode) {
    return mode === "ideas" ? 5 : 3;
}
export function defaultOutputPath(cwd, mode, slug) {
...
//# sourceMappingURL=cursor-duel.js.map