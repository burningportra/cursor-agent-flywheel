...
function findReferent(name, stepBody) {
    const escaped = escapeRegExp(name);
    return (new RegExp(`\\*\\*${escaped}\\*\\*`, "i").test(stepBody) ||
        new RegExp(`(^|\\n)\\s*[-*]\\s+\\*\\*${escaped}\\*\\*\\s*:`, "i").test(stepBody) ||
        new RegExp(`\\b${escaped}\\s*[=:]`, "i").test(stepBody));
}
function enclosingHeader(line, headers) {
    let best;
...
function stepBodyText(source, header, headers) {
    const startOffset = header.span.end.offset ?? 0;
    const next = headers.find((h) => h.span.start.line > header.span.start.line && h.level <= header.level);
    const endOffset = next?.span.start.offset ?? source.length;
    return source.slice(startOffset, endOffset);
}
export const place001 = {
    id: "PLACE001",
    description: "Placeholder tags (e.g. <USER_INPUT>) must be defined or referenced in the enclosing step.",
    severity: "warn",
    check(doc, ctx) {
        const findings = [];
        const parsed = doc;
