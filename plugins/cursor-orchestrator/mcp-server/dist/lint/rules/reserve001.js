...
async function walkTsFiles(root, opts) {
    const out = [];
    let entries;
    try {
...
export const reserve001 = {
    id: "RESERVE001",
    description: "Direct agentMailRPC file-reservation calls must route through reserveOrFail() in agent-mail-helpers.ts.",
    severity: "warn",
    async check(_doc, ctx) {
        const rc = ctx;
        ...
        for (const file of files) {
            if (isAllowlisted(file.rel, allowlist))
                continue;
            let source;
            try {
            ...
            if (!source.includes("file_reservation_paths"))
                continue;
            if (!source.includes("agentMailRPC"))
                continue;
            ...
            while ((match = RESERVE_CALL_PATTERN.exec(source)) !== null) {
                ...
                findings.push({
                    ...
                    line,
                    column,
                    message: 'Direct agentMailRPC("file_reservation_paths") call. Route through reserveOrFail() in mcp-server/src/agent-mail-helpers.ts so non-empty conflicts are treated as failure (AGENTS.md "Known issue").',
...
//# sourceMappingURL=reserve001.js.map