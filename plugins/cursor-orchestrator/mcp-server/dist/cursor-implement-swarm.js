...
export function recommendImplModels(cwd, beads = []) {
    ...
    if (complexShare >= 0.25 ||
        preview.complex >= 2 ||
        (hardShare >= 0.6 && preview.complex >= 1)) {
        const models = {
            simple: baseline.simple,
            medium: baseline.medium !== baseline.simple ? baseline.medium : "gpt-5.5-xhigh",
...
export function buildImplModelsGate(cwd, beads = []) {
    ...
    const { models: recommended, rationale, preview } = recommendImplModels(cwd, beads);
...
export function buildCursorImplSpawnInstructions(models) {
    return [
...
export function adaptPromptForCursor(bead, taskModel) {
