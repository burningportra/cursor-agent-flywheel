...
export class SwarmTender {
        ...
        for (const [stepIndex, agent] of this.agents) {
            try {
                ...
                for (const file of files) {
                    ...
                    if (file.startsWith(".pi-orchestrator/"))
                        continue;
    ...
    async nudgeStuckAgent(stuckAgentName, threadId) {
        if (!this.orchestratorAgentName)
            return;
        await sendMessage(this.exec, this.cwd, this.orchestratorAgentName, [stuckAgentName], `[SwarmTender] Are you stuck?`, `You haven't made changes in >${this.config.stuckThreshold / 1000}s. ` +
