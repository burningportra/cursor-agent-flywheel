...
describe('chaos/bundle-not-found', () => {
    it('throws FlywheelError with code not_found when skill absent from bundle and disk', async () => {
        ...
        await expect(getSkill('agent-flywheel:does-not-exist', { repoRoot: cwd, bundlePath })).rejects.toThrow(FlywheelError);
        await expect(getSkill('agent-flywheel:does-not-exist', { repoRoot: cwd, bundlePath })).rejects.toMatchObject({ code: 'not_found' });
    });
    it('throws not_found with no bundle present and skill absent from disk', async () => {
        await expect(getSkill('agent-flywheel:ghost', {
            repoRoot: cwd,
...
//# sourceMappingURL=bundle-not-found.test.js.map