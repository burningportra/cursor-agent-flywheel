...
function spawnFakeParentWithViewer() {
  ...
  `;
    return new Promise((resolveR, rejectR) => {
        ...
        const t = setTimeout(() => {
            ...
            rejectR(new Error(`fake-parent setup timed out after ${FAKE_PARENT_TIMEOUT_MS}ms; stdout=${stdout} stderr=${stderr}`));
        }, FAKE_PARENT_TIMEOUT_MS);
        fakeParent.stderr?.on('data', (b) => {
            stderr += b.toString();
        });
        fakeParent.stdout?.on('data', (b) => {
            stdout += b.toString();
            if (viewerPid === null) {
                const m = stdout.match(/VIEWER_PID:(\d+)/);
                if (m)
                    viewerPid = Number.parseInt(m[1], 10);
            }
        ...
        fakeParent.on('exit', (code, signal) => {
            if (viewerPid === null || !ready) {
                clearTimeout(t);
                rejectR(new Error(`fake parent exited before viewer ready (code=${code} signal=${signal}); stdout=${stdout} stderr=${stderr}`));
            }
        });
    });
}
...
//# sourceMappingURL=viewer-parent-death.test.js.map