...
import { RECOVER_BEAD_SCAN_TIMEOUT_MS, RECOVER_CANDIDATE_CAP, RECOVER_CHECKPOINT_STALE_MS, RECOVER_WARNING_CAP, classifyCheckpointTrust, degradeToManual, extractCheckpointCandidateIds, resolveRecoveryContext, scanBeadCandidates, } from "../recover-gates.js";
...
function makeBead(id, status = "closed") {
    return {
        id,
        title: "Test bead",
        description: "desc",
...
function makeCtx(execCalls = [], stateOverrides = {}, cwd) {
    return {
...
describe("recover-gates", () => {
        ...
        it("collects success and legacy closed beadResults", () => {
            const state = makeState({
                beadResults: {
                    ...
                    "tb-b": { status: "closed", reviewPasses: 1 },
                },
            });
    ...
    describe("scanBeadCandidates", () => {
        it("returns closed beads capped at RECOVER_CANDIDATE_CAP", async () => {
            const beads = Array.from({ length: 30 }, (_, i) => makeBead(`tb-${i}`, "closed"));
            const ctx = makeCtx([
                {
                    cmd: "br",
                    args: BR_LIST_ARGS,
        ...
        it("defaults timeout and cap to RECOVER_* constants", async () => {
            const beads = Array.from({ length: 30 }, (_, i) => makeBead(`tb-default-${i}`, "closed"));
            const ctx = makeCtx([
                {
                    cmd: "br",
                    args: BR_LIST_ARGS,
    ...
    describe("resolveRecoveryContext", () => {
        ...
        it("caps bead_scan candidates at RECOVER_CANDIDATE_CAP via resolveRecoveryContext", async () => {
            ...
            const beads = Array.from({ length: 30 }, (_, i) => makeBead(`tb-cap-${i}`, "closed"));
            const ctx = makeCtx([
                {
                    cmd: "br",
                    args: BR_LIST_ARGS,
...
//# sourceMappingURL=recover-gates.test.js.map