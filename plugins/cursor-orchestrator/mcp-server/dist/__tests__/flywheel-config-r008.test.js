...
import { loadFlywheelConfig, loadFlywheelConfigWithWarnings, suggestKey, DEFAULT_CONFIG, } from '../flywheel-config.js';
...
describe('R-008 — loadFlywheelConfigWithWarnings', () => {
    ...
    it('typo "convergance" surfaces unknown_key warning with suggestion="convergence"', () => {
        writeFileSync(join(tmp, 'flywheel.config.yaml'), `convergance:\n  gate_advance_wave: false\n`);
        const result = loadFlywheelConfigWithWarnings(tmp);
    ...
    it('completely-unknown key warns without a suggestion', () => {
        writeFileSync(join(tmp, 'flywheel.config.yaml'), `totally_made_up_section:\n  whatever: true\n`);
        const result = loadFlywheelConfigWithWarnings(tmp);
        expect(result.warnings).toHaveLength(1);
        expect(result.warnings[0]).toMatchObject({
            kind: 'unknown_key',
