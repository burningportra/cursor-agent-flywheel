...
describe('skills/start/SKILL.md — Step 0d menu options (claude-orchestrator-1o4)', () => {
    ...
    for (const variant of VARIANTS) {
        describe(`menu variant: ${variant.name}`, () => {
            ...
            it.each(REQUIRED_LABELS)(`surfaces "%s" in the AskQuestion options table`, (label) => {
                const tablePattern = new RegExp(`\\|\\s*[^|]+\\s*\\|\\s*${escapeRegex(label)}(?:\\s+\\(Recommended\\))?\\s*\\|`);
                expect(tablePattern.test(section), `expected AskQuestion table row with label "${label}" in ${variant.name} menu`).toBe(true);
            });
        });
    }
    ...
    describe('Step 5.45 — picked-up-plan menu', () => {
        ...
        it('Pick up existing plan rows in all 3 menu variants mention Step 5.45 / Validate', () => {
            const pickPlanRows = skillBody.match(/\|\s*pick-plan\s*\|\s*Pick up existing plan\s*\|[^|\n]*/g);
            expect(pickPlanRows?.length ?? 0, 'expected 3 pick-plan table rows (one per menu variant)').toBe(3);
            for (const row of pickPlanRows ?? []) {
                expect(row.includes('5.45') || row.toLowerCase().includes('validate'), `Pick-up row should mention Step 5.45 or validate: "${row}"`).toBe(true);
            }
        });
    });
});
