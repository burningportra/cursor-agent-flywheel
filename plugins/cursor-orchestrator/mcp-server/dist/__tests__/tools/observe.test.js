...
function writeAttestationFixture(cwd, beadId, createdAt) {
    ...
    writeFileSync(join(cwd, '.pi-flywheel', 'completion', `${beadId}.json`), JSON.stringify(report, null, 2));
}
describe('flywheel_observe — T7 attestation hints', () => {
    it('surfaces only the missing attestation when 1 of 2 in-flight beads has one', async () => {
        ...
        try {
            ...
            const missingHint = report.hints.find((h) => h.severity === 'warn' && h.message.includes('bead-missing'));
            ...
            const attestedHint = report.hints.find((h) => h.message.includes('bead-attested'));
    ...
    it('emits info hint for stale attestation (createdAt > 24h ago)', async () => {
        ...
        try {
            ...
            const staleHint = report.hints.find((h) => h.severity === 'info' && h.message.includes('bead-stale'));
    ...
    it('classifies invalid-JSON attestation as invalid (not missing) with warn hint', async () => {
        ...
        try {
            ...
            mkdirSync(join(cwd, '.pi-flywheel', 'completion'), { recursive: true });
            writeFileSync(join(cwd, '.pi-flywheel', 'completion', 'bead-broken.json'), '{not valid json');
            const exec = makeStubbedExec(gracefulDegradeStubs());
            const result = await runObserve(makeCtx(cwd, exec), { cwd });
            const sc = result.structuredContent;
            const report = sc.data.report;
            ...
            expect(report.attestations.missing).toEqual([]);
            const invalidHint = report.hints.find((h) => h.severity === 'warn' && h.message.includes('schema validation'));
