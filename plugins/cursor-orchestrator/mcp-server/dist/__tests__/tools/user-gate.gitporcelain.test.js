...
const renameFixture = readFileSync(join(__dirname, '../fixtures/git-status-rename.txt'), 'utf8');
...
const { parseGitPorcelainZ, runWrapUpGate } = await import('../../tools/user-gate.js');
...
describe('gitPorcelain via runWrapUpGate', () => {
    ...
    it('uses porcelain v1 -z and surfaces rename paths in wrap-up preview', async () => {
        ...
        const data = result.structuredContent
            .data;
        const rationale = data.gateMeta.rationale;
        expect(execFileMock).toHaveBeenCalledWith('git', ['status', '--porcelain=v1', '-z'], expect.objectContaining({ cwd: '/fake/project' }), expect.any(Function));
    ...
    it('returns empty preview when git fails', async () => {
        ...
        const data = result.structuredContent
            .data;
        const rationale = data.gateMeta.rationale;
        expect(rationale).toContain('Working tree is clean aside from any intentional WIP.');
    });
});
//# sourceMappingURL=user-gate.gitporcelain.test.js.map