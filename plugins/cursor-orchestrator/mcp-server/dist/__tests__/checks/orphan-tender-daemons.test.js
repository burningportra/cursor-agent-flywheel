...
import { ORPHAN_TENDER_DAEMONS_CHECK_NAME, checkOrphanTenderDaemons, classifyOrphans, parseTenderDaemonProcesses, parseTmuxSessions, } from '../../checks/orphan-tender-daemons.js';
// ─── pure parsers ─────────────────────────────────────────────────────────
...
function makeExec(calls) {
    return async (cmd, args) => {
        const match = calls.find((c) => c.cmd === cmd && c.args.length === args.length && c.args.every((a, i) => a === args[i]));
        if (!match) {
...
//# sourceMappingURL=orphan-tender-daemons.test.js.map