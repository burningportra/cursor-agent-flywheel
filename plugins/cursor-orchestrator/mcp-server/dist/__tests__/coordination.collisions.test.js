...
import { captureWaveStartSha, diffWorkerAgainstWaveStart, matchesGlob, isIgnoredCollisionPath, aggregateCollisions, detectWaveCollisions, forceSerialRerun, collidingUnitIds, loadCollisionIgnore, seedCollisionIgnore, DEFAULT_COLLISION_IGNORE, COLLISION_IGNORE_PATH, WAVE_COLLISION_HINT, } from "../coordination.js";
...
describe("detectWaveCollisions", () => {
    ...
    it("aggregates per-worker diffs into a collision report", async () => {
        ...
        const report = await detectWaveCollisions(exec, tmpRoot, "shaWAVE", [
    ...
    it("falls back to DEFAULT_COLLISION_IGNORE when no ignore file exists", async () => {
        ...
        const report = await detectWaveCollisions(exec, tmpRoot, "wave1", [
    ...
    it("honors a custom .pi-flywheel/collision-ignore file", async () => {
        ...
        writeFileSync(join(tmpRoot, COLLISION_IGNORE_PATH), "# custom\nsrc/shared.ts\n", "utf-8");
        let i = 0;
        const responses = [
            { code: 0, stdout: "src/shared.ts\nsrc/a.ts\n", stderr: "" },
            { code: 0, stdout: "src/shared.ts\nsrc/b.ts\n", stderr: "" },
...
describe("seedCollisionIgnore / loadCollisionIgnore", () => {
    ...
    it("loadCollisionIgnore strips comments and blank lines", () => {
        mkdirSync(join(tmpRoot, ".pi-flywheel"), { recursive: true });
        writeFileSync(join(tmpRoot, COLLISION_IGNORE_PATH), "# comment\n\nfoo.ts\n  bar.ts  \n# trailing comment\n", "utf-8");
...
//# sourceMappingURL=coordination.collisions.test.js.map