...
import { appendRevision, computeRevisionMetrics, ConvergenceStateSchema, } from '../convergence.js';
import { convergencePath, readConvergenceFromDisk, runConvergence, writeConvergenceToDisk, planSlugFromIdentifier, } from '../tools/convergence-tool.js';
import { loadFlywheelConfig, DEFAULT_CONFIG, } from '../flywheel-config.js';
...
function makeCtx(cwd, state = makeState()) {
    return {
        cwd,
        state,
        saveState: () => { },
...
function buildConvergenceFixture(planSlug, score) {
...
describe('B-AC2 — flywheel_convergence MCP tool', () => {
    ...
    it('writeConvergenceToDisk refuses to write a state with mismatched scoreVersion', async () => {
        ...
        malformed.scoreVersion = 999;
        await expect(writeConvergenceToDisk(tmpRoot, malformed)).rejects.toThrow();
    });
...
describe('B-AC2 — flywheel_observe schema parity + convergence wiring', () => {
    ...
    it('convergenceGated reflects flywheel.config.yaml kill-switch state', async () => {
        writeFileSync(join(tmpRoot, 'flywheel.config.yaml'), 'convergence:\n  gate_advance_wave: false\n', 'utf8');
        const ctx = makeCtx(tmpRoot);
        const result = await runObserve(ctx, { cwd: tmpRoot });
        const structured = result.structuredContent;
...
//# sourceMappingURL=convergence-integration.test.js.map