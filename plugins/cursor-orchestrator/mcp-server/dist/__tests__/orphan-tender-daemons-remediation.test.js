...
import { remediateOrphanDaemons, orphanTenderDaemonsHandler, } from '../tools/remediations/orphan_tender_daemons.js';
 ...
 */
function makeKillFn(behavior) {
    // Track liveness state per pid across calls.
    const state = new Map(); // true=alive, false=gone
    ...
    return (pid, signal) => {
        ...
        if (signal === 'SIGTERM') {
            if (kind === 'esrch-immediately')
                return false;
            if (kind === 'gone-after-term')
                state.set(pid, false);
...
const fastSleep = (_ms) => Promise.resolve();
...
describe('orphanTenderDaemonsHandler (T6.2) — doctor integration', () => {
    ...
    it('buildPlan returns an empty plan when ps lists no tender-daemons', async () => {
        ...
        const ctx = { cwd: '/tmp', exec, signal: new AbortController().signal };
...
//# sourceMappingURL=orphan-tender-daemons-remediation.test.js.map