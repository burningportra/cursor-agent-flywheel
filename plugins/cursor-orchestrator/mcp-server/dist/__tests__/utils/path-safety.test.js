...
import { assertSafeRelativePath, assertSafeSegment, requireSafeRelativePath, requireSafeSegment, resolveRealpathWithinRoot, } from "../../utils/path-safety.js";
...
describe("resolveRealpathWithinRoot", () => {
    ...
    it("rejects with reason 'root_not_found' when root itself is missing", () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.reason).toBe("root_not_found");
    });
});
