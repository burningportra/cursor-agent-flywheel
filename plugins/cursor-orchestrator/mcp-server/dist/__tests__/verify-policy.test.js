...
import { BUILD_SLOT_NAME, DEFAULT_VERIFY_POLICY, formatFullSuiteTestCommand, formatScopedTestCommand, formatSwarmVerifyBlock, verifyPolicyFromConfig, } from '../verify-policy.js';
...
describe('formatScopedTestCommand', () => {
    it('includes file paths when provided', () => {
        expect(formatScopedTestCommand(['src/a.test.ts', 'src/b.test.ts'])).toBe(`cd ${DEFAULT_VERIFY_POLICY.testCwd} && npm test -- src/a.test.ts src/b.test.ts`);
    });
...
describe('formatFullSuiteTestCommand', () => {
    it('runs npm test in test cwd', () => {
        expect(formatFullSuiteTestCommand()).toBe(`cd ${DEFAULT_VERIFY_POLICY.testCwd} && npm test`);
    });
});
...
//# sourceMappingURL=verify-policy.test.js.map