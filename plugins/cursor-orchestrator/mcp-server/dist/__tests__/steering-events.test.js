...
import { STEERING_EVENTS_CAP, appendSteeringEvent, buildSteeringNormalizedKey, recordGateSteering, shouldSuppressNextActionHint, } from '../steering-events.js';
...
describe('buildNextActionHint suppression integration', () => {
    it('omits advance_wave hint when three skip events share normalizedKey', () => {
        ...
        expect(buildNextActionHint('advance_wave', 1, { state, beadIds, beadCount: 2 })).toBeUndefined();
    });
});
describe('recordGateSteering checkpoint persistence', () => {
    it('persists steeringEvents through saveState and loadState', async () => {
        ...
        expect(restored.steeringEvents[0].normalizedKey).toBe(buildSteeringNormalizedKey('skip', ['tb-9']));
        expect(restored.coordinatorEpoch).toBe(1);
    });
});
//# sourceMappingURL=steering-events.test.js.map