...
const noChangesExec = vi.fn().mockResolvedValue({ code: 0, stdout: '', stderr: '' });
/** exec that simulates a worktree with changed files. */
function changedFilesExec(files) {
...
function makeTender(worktrees, options = {}, exec = noChangesExec) {
...
describe('SwarmTender — auto-escalation (flywheelAgentName set)', () => {
    ...
    it('kills agent after maxNudges + killWaitMs with no activity', async () => {
        ...
        const killedAgent = onKill.mock.calls[0][0];
        ...
        const summary = onSwarmComplete.mock.calls[0][0];
...
describe('SwarmTender — removeAgent and onSwarmComplete', () => {
    it('fires onSwarmComplete when the last agent is removed normally', () => {
        const onSwarmComplete = vi.fn();
        const tender = makeTender([{ path: '/wt/0', stepIndex: 0 }, { path: '/wt/1', stepIndex: 1 }], { onSwarmComplete });
...
describe('loadTenderConfig', () => {
    ...
    it('applies JSON file overrides', () => {
        ...
        fs.writeFileSync(path.join(cfgDir, 'tender.config.json'), JSON.stringify({ pollInterval: 1234, maxNudgesPerPoll: 7 }));
        const cfg = loadTenderConfig(tmpDir);
    ...
    it('env var wins over file on conflict', () => {
        const cfgDir = path.join(tmpDir, '.pi-flywheel');
        fs.mkdirSync(cfgDir, { recursive: true });
        fs.writeFileSync(path.join(cfgDir, 'tender.config.json'), JSON.stringify({ pollInterval: 1111, maxNudges: 4 }));
    ...
    it('ignores unknown JSON keys and non-numeric values', () => {
        const cfgDir = path.join(tmpDir, '.pi-flywheel');
        fs.mkdirSync(cfgDir, { recursive: true });
        fs.writeFileSync(path.join(cfgDir, 'tender.config.json'), JSON.stringify({
            notARealKey: 42,
...
describe('SwarmTender — telemetry integration', () => {
    ...
    it('emits at least one nudge_sent and one poll_summary event to the log', async () => {
        ...
        const tender = new SwarmTender(exec, tmp, [{ path: '/wt/0', stepIndex: 0 }], {
            config: {
                stuckThreshold: 1_000,
                idleThreshold: 500,
                pollInterval: 999_999,
...
//# sourceMappingURL=tender.test.js.map