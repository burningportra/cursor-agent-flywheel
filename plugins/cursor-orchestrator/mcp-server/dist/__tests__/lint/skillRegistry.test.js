...
async function writeSkill(root, dir, name) {
...
describe("loadSkillRegistry", () => {
    ...
    describe("layer 2 (manifest)", () => {
        it("loads skills from a valid manifest file", async () => {
            ...
            await writeFile(manifestAbs, JSON.stringify({ schemaVersion: 1, skills: ["m1", "m2"] }), "utf8");
        ...
        it("ignores manifest with wrong schemaVersion", async () => {
            const manifestAbs = path.join(tmp, "manifest.json");
            await writeFile(manifestAbs, JSON.stringify({ schemaVersion: 2, skills: ["x"] }), "utf8");
            const reg = await loadSkillRegistry({
                repoRoot: tmp,
                ci: true,
                pluginsRoot: null,
    ...
    describe("layer 3 (allowlist)", () => {
        it("normalizes leading slashes from knownExternalSlashes", async () => {
            ...
            await writeFile(allowlistAbs, JSON.stringify({ schemaVersion: 1, knownExternalSlashes: ["fast", "/clear"] }), "utf8");
            const reg = await loadSkillRegistry({
                repoRoot: tmp,
                ci: true,
                pluginsRoot: null,
        ...
        it("ignores allowlist with wrong schemaVersion", async () => {
            ...
            await writeFile(allowlistAbs, JSON.stringify({ schemaVersion: 9, knownExternalSlashes: ["x"] }), "utf8");
            const reg = await loadSkillRegistry({
                repoRoot: tmp,
                ci: true,
                pluginsRoot: null,
    ...
    describe("layer 4 (plugins, dev mode)", () => {
        ...
        it("HOME=/nonexistent: pluginsRoot pointing nowhere does not throw", async () => {
            ...
            const manifestAbs = path.join(tmp, "manifest.json");
            await writeFile(manifestAbs, JSON.stringify({ schemaVersion: 1, skills: ["mfest"] }), "utf8");
            const allowlistAbs = path.join(tmp, "allow.json");
            await writeFile(allowlistAbs, JSON.stringify({ schemaVersion: 1, knownExternalSlashes: ["allowed"] }), "utf8");
            const reg = await loadSkillRegistry({
                repoRoot: tmp,
                ci: false,
        ...
        it("integration: ci=true with pluginsRoot=/nonexistent returns successfully", async () => {
            ...
            const manifestAbs = path.join(tmp, "manifest.json");
            await writeFile(manifestAbs, JSON.stringify({ schemaVersion: 1, skills: ["m"] }), "utf8");
            const allowlistAbs = path.join(tmp, "allow.json");
            await writeFile(allowlistAbs, JSON.stringify({ schemaVersion: 1, knownExternalSlashes: ["a"] }), "utf8");
            const reg = await loadSkillRegistry({
                repoRoot: tmp,
                ci: true,
    ...
    describe("Levenshtein suggest", () => {
        it("returns nearest match first", async () => {
            const allowlistAbs = path.join(tmp, "allow.json");
            await writeFile(allowlistAbs, JSON.stringify({
                schemaVersion: 1,
        ...
        it("filters distance > 5 as irrelevant", async () => {
            const allowlistAbs = path.join(tmp, "allow.json");
            await writeFile(allowlistAbs, JSON.stringify({ schemaVersion: 1, knownExternalSlashes: ["abc"] }), "utf8");
            const reg = await loadSkillRegistry({
                repoRoot: tmp,
                ci: true,
                pluginsRoot: null,
    ...
    describe("priority and merging", () => {
        it("first layer to register a name wins for source()", async () => {
            ...
            const manifestAbs = path.join(tmp, "manifest.json");
            await writeFile(manifestAbs, JSON.stringify({ schemaVersion: 1, skills: ["shared"] }), "utf8");
            const reg = await loadSkillRegistry({
                repoRoot: tmp,
                ci: true,
                pluginsRoot: null,
