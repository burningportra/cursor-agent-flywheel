...
import { appendRevision, computeRevisionMetrics, ConvergenceStateSchema, defaultStep545Action, parseConvergenceState, REVISION_BUFFER_SIZE, SCORE_VERSION, } from "../convergence.js";
 ...
 */
function driveSequence(lineCounts, opts = {}) {
    const states = [];
    let state = null;
    let prior = null;
...
describe("convergence — status ladder", () => {
    ...
    it("monotonic-converging sequence transitions through ladder without skipping", () => {
        ...
        for (const s of seenStatuses) {
            if (s === "oscillating")
                continue; // not expected here, but guarded
...
describe("convergence — appendRevision edge cases", () => {
    ...
    it("only branches on score and status — never on revisions.length", () => {
        ...
        expect(defaultStep545Action({ ...baseState, score: 0.8, status: "nearly_converged" })).toBe("validate");
        expect(defaultStep545Action({ ...baseState, score: 0.4, status: "diverging" })).toBe("refine");
        expect(defaultStep545Action({
            ...baseState,
            status: "oscillating",
            score: 0.95,
