...
describe('collectReviewVerdict', () => {
    ...
    it('pass verdict returns proceed/advance next step', async () => {
        vi.mocked(fsPromises.readFile).mockResolvedValueOnce(JSON.stringify({ status: 'pass', findings: [], sha_range: SHA_RANGE }));
        const { ctx } = makeCtx();
    ...
    it('blocking verdict synthesizes beads with labels', async () => {
        ...
        vi.mocked(fsPromises.readFile).mockResolvedValueOnce(JSON.stringify({ status: 'blocking', findings, sha_range: SHA_RANGE }));
        ...
        expect(vi.mocked(synthesizeBeadsFromFindings)).toHaveBeenCalledWith('/fake/cwd', expect.any(Object), findings, SHA_RANGE, ['auto-review-finding']);
    });
...
//# sourceMappingURL=review-verdict-collect.test.js.map