...
import { DEFAULT_ALLOWED_HOSTS, CloneSafetyError, validateCloneUrl, validateGitRef, safeClone, formatProvenance, } from '../utils/clone-safety.js';
describe('validateCloneUrl', () => {
    ...
    it('allows non-https when FLYWHEEL_ALLOW_INSECURE_CLONE=1', () => {
        expect(() => validateCloneUrl('http://github.com/foo/bar', {
            env: { FLYWHEEL_ALLOW_INSECURE_CLONE: '1' },
        })).not.toThrow();
    });
    it('rejects file:// even with insecure bypass', () => {
        expect(() => validateCloneUrl('file:///etc/passwd', {
            env: { FLYWHEEL_ALLOW_INSECURE_CLONE: '1' },
        })).toThrow(/protocol not allowed/);
    });
    it('rejects javascript: URLs', () => {
        expect(() => validateCloneUrl('javascript:alert(1)', {
            env: { FLYWHEEL_ALLOW_INSECURE_CLONE: '1' },
        })).toThrow();
    });
    it('rejects hosts not in allowlist', () => {
        expect(() => validateCloneUrl('https://evil.example.com/foo/bar')).toThrow(/not in allowlist/);
    });
    it('accepts hosts added via extraAllowedHosts (e.g. GHE)', () => {
        expect(() => validateCloneUrl('https://ghe.corp.example/foo/bar', {
            extraAllowedHosts: ['ghe.corp.example'],
        })).not.toThrow();
    });
    it('rejects URLs with embedded credentials', () => {
        expect(() => validateCloneUrl('https://user:pass@github.com/foo/bar')).toThrow(/embedded credentials/);
    });
...
describe('safeClone', () => {
    function makeMockExec(rev = 'abcdef1234567890abcdef1234567890abcdef12') {
        const calls = [];
        const exec = vi.fn(async (cmd, args, _opts) => {
            calls.push({ cmd, args });
    ...
    it('rejects malicious branch ref', async () => {
        ...
        await expect(safeClone(exec, 'https://github.com/foo/bar.git', '/tmp/x', {
            branch: '--upload-pack=evil',
        })).rejects.toThrow(CloneSafetyError);
    });
    it('rejects non-allowed host without touching exec', async () => {
        const exec = vi.fn();
        await expect(safeClone(exec, 'https://evil.example.com/foo/bar', '/tmp/x')).rejects.toThrow(/allowlist/);
    ...
    it('throws CloneSafetyError when clone exits non-zero', async () => {
        ...
        await expect(safeClone(exec, 'https://github.com/foo/bar.git', '/tmp/x')).rejects.toThrow(/clone failed/);
    });
    it('throws when HEAD SHA is not a valid git object id', async () => {
        ...
        await expect(safeClone(exec, 'https://github.com/foo/bar.git', '/tmp/x')).rejects.toThrow(/object id/);
    });
});
...
//# sourceMappingURL=clone-safety.test.js.map