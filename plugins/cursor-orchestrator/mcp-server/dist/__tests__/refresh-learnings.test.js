...
import { parseSolutionDocMarkdown, scoreOverlap, groupSolutionDocs, classifyGroup, refreshLearnings, } from '../refresh-learnings.js';
import { renderSolutionDoc, } from '../solution-doc-schema.js';
// ─── Helpers ───────────────────────────────────────────────────
function makeDoc(overrides) {
    const { path = 'docs/solutions/test/sample-2026-04-23.md', body = '## What shipped\n\nFixed it.', ...fm } = overrides;
    return {
        path,
        frontmatter: {
            entry_id: 'cass-1',
...
describe('scoreOverlap', () => {
    ...
    it('produces per-dimension scores in [0,1]', () => {
        ...
        const score = scoreOverlap(a, b);
        for (const dim of ['problem', 'rootCause', 'solution', 'files', 'prevention', 'overall']) {
...
//# sourceMappingURL=refresh-learnings.test.js.map