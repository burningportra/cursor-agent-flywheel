...
import { _resetGraderMutex, defaultGraderDriver, } from '../outcome-grading.js';
const SAMPLE_RUBRIC = `---
version: 1
source: auto
...
function makeGrader(stdout, modelUsed = 'codex') {
...
function setupCtxAndRubric(slug, stateOverrides = {}) {
    ...
    const exec = async (cmd, args) => {
        if (cmd === 'git' && args[0] === 'log')
            return { code: 0, stdout: 'abc1234 commit', stderr: '' };
...
describe('flywheel_grade_outcome tool wrapper', () => {
    ...
    it('grader_verdict: happy path returns version 1 envelope with verdict', async () => {
        ...
        const spy = (gradeFn) => async (ctx, args) => gradeFn(ctx, args, { grader: makeGrader(makeVerdictJson()) });
    ...
    it('Zod input failure surfaces invalid_input', async () => {
        ...
        dir = setup.dir;
        const result = await runGradeOutcome(setup.ctx, { cwd: 12 });
        expect(result.isError).toBe(true);
        const sc = result.structuredContent;
...
//# sourceMappingURL=grade-outcome.test.js.map