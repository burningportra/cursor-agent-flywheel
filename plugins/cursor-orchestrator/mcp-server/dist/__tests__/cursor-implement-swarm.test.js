...
import { DEFAULT_CURSOR_IMPL_MODELS, adaptPromptForCursor, buildBeadDispatchContext, buildCursorImplSpawnInstructions, classifyBeadsForSwarm, buildImplModelsGate, getCursorImplModels, modelForComplexity, recommendImplModels, resolveImplModelsConfirm, useNtmImplBackend, } from '../cursor-implement-swarm.js';
function bead(title, description = '') {
    return {
        id: 'b-1',
        title,
        description,
...
describe('cursor-implement-swarm', () => {
    ...
    it('reads implement section from flywheel.config.yaml', () => {
        writeFileSync(join(tmp, 'flywheel.config.yaml'), 'implement:\n  simple: fast-model\n  medium: mid-model\n  complex: heavy-model\n');
    ...
    it('resolveImplModelsConfirm handles defaults and uniform', () => {
        expect(resolveImplModelsConfirm(tmp, 'defaults')).toEqual(DEFAULT_CURSOR_IMPL_MODELS);
    ...
    it('recommendImplModels upgrades tier when queue is complex-heavy', () => {
        const heavyDesc = 'security authentication refactor integration concurrent distributed protocol architect cross-cutting breaking change state machine cryptographic migration '.repeat(40);
        const beads = [
    ...
    it('resolveImplModelsConfirm recommended matches recommendImplModels', () => {
        ...
        expect(resolveImplModelsConfirm(tmp, 'recommended', beads)).toEqual(recommendImplModels(tmp, beads).models);
    });
    ...
    it('adaptPromptForCursor includes single-branch git workflow and no worktrees', () => {
        const ctx = buildBeadDispatchContext({
            ...
            id: 'br-99',
        }, 'medium', 'AgentOne', 'Coordinator', tmp);
    ...
    it('buildBeadDispatchContext extracts artifact paths from bead body', () => {
        const ctx = buildBeadDispatchContext(bead('Docs', '### Files:\n- docs/readme.md'), 'simple', 'Agent', 'Coordinator', tmp);
        expect(ctx.relevantFiles).toContain('docs/readme.md');
    });
});
//# sourceMappingURL=cursor-implement-swarm.test.js.map