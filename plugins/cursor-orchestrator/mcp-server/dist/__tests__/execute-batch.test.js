...
import { mkdtemp, readFile, writeFile, rm, readlink } from "node:fs/promises";
import { existsSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { executeBatch, performSymlink, registerMcpAtomic, } from "../setup-detector.js";
/**
...
describe("registerMcpAtomic", () => {
    ...
    it("preserves existing mcpServers entries when merging", async () => {
        await writeFile(cfg, JSON.stringify({ mcpServers: { other: { command: "x" } } }, null, 2));
        const result = await registerMcpAtomic({ configPath: cfg, pluginRoot: "/plugin" });
        expect(result.status).toBe("ok");
        const parsed = JSON.parse(await readFile(cfg, "utf8"));
