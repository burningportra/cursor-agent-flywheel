...
function makeFixture(versions) {
    ...
    if (versions.local) {
        ...
        writeFileSync(join(repo, 'mcp-server', 'package.json'), JSON.stringify({ name: 'mcp', version: versions.local }, null, 2) + '\n');
    }
    if (versions.marketplace || versions.pluginManifest) {
        mkdirSync(join(repo, '.claude-plugin'), { recursive: true });
        if (versions.pluginManifest) {
            writeFileSync(join(repo, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'agent-flywheel', version: versions.pluginManifest }, null, 2) + '\n');
        }
        if (versions.marketplace) {
            writeFileSync(join(repo, '.claude-plugin', 'marketplace.json'), JSON.stringify({ name: 'mp', version: versions.marketplace }, null, 2) + '\n');
        }
    }
...
describe('version-sync.mjs', () => {
    ...
    it('preserves indent style when rewriting JSON files', () => {
        ...
        mkdirSync(join(repo, '.claude-plugin'), { recursive: true });
        writeFileSync(join(repo, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'agent-flywheel', version: '3.10.0' }, null, 4) + '\n');
    ...
    it('--help prints usage and exits 0', () => {
        repo = mkdtempSync(join(tmpdir(), 'version-sync-test-'));
        const cfg = join(repo, 'cfg.json');
        writeFileSync(cfg, JSON.stringify({ manifests: [{ path: 'mcp-server/package.json', isSource: true }] }));
...
//# sourceMappingURL=version-sync.test.js.map