...
import { ROBOT_DOCS_VERSION, ROBOT_DOCS_SECTIONS, buildRobotDocs, runRobotDocs, } from '../tools/robot-docs.js';
describe('flywheel_robot_docs — section coverage', () => {
    ...
    it('runRobotDocs falls back to "all" on unknown section', async () => {
        const res = await runRobotDocs({ cwd: '/tmp', exec: null, signal: null }, { section: 'made_up_section' });
    ...
    it('runRobotDocs envelope shape is {tool,version,status,phase,data}', async () => {
        const res = await runRobotDocs({ cwd: '/tmp', exec: null, signal: null }, { section: 'getting_started' });
        const sc = res.structuredContent;
...
//# sourceMappingURL=robot-docs.test.js.map