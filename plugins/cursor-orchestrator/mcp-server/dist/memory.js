 ...
 */
export function detectCass() {
    const now = Date.now();
    if (_cassAvailable === true)
        return true;
...
export function resetCassDetection() {
    _cassAvailable = null;
    _cassCheckedAt = 0;
}
// ─── Helpers ────────────────────────────────────────────────
function runCm(args, cwd) {
    try {
 ...
 */
export function getContext(task, cwd) {
    if (!detectCass())
        return null;
    const output = runCm(["context", task, "--json"], cwd);
    return parseCmJson(output);
}
/**
 ...
 */
export function appendMemory(cwd, entry, category) {
    if (!detectCass())
        return false;
 ...
 */
export function listMemoryEntries(cwd) {
    if (!detectCass())
        return [];
 ...
 */
export function markRule(bulletId, helpful, reason, cwd) {
    if (!detectCass())
        return false;
 ...
 */
export function reflectMemory(cwd) {
    if (!detectCass())
        return false;
 ...
 */
export function skillRefinerPrompt(skillContent, skillName, sessionSnippets) {
...
export function getMemoryStats(cwd) {
    ...
    const statsData = parseCmJson(statsOutput);
    return {
