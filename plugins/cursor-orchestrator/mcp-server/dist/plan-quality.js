 ...
 */
export function planQualityScoringPrompt(plan, goal) {
 ...
 */
export function parsePlanQualityScore(output) {
    ...
    try {
        ...
        const weakSections = Array.isArray(parsed.weakSections)
            ? parsed.weakSections.filter((s) => typeof s === "string").slice(0, 10)
            : [];
        // Weighted average: workflows 25%, edgeCases 20%, architecture 20%, specificity 20%, testability 15%
        const overall = Math.round(workflows * 0.25 +
            edgeCases * 0.20 +
            architecture * 0.20 +
            specificity * 0.20 +
            testability * 0.15);
        const recommendation = overall < 60 ? "block" : overall < 80 ? "warn" : "proceed";
        return {
            overall,
            workflows,
            edgeCases,
 ...
 */
export function formatPlanQualityScore(score) {
    const bar = (value) => {
...
//# sourceMappingURL=plan-quality.js.map