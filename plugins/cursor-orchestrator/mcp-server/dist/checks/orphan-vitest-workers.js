...
export const ORPHAN_VITEST_WORKERS_CHECK_NAME = 'orphan_vitest_workers';
const VITEST_CMD = /\b(vitest|npm\s+(run\s+)?test\b|pnpm\s+(run\s+)?test\b|yarn\s+(run\s+)?test\b)/i;
/** Parse `ps -eo pid,ppid,etime,command` lines into vitest-related processes. */
export function parseVitestProcesses(psOutput) {
    const out = [];
    for (const line of psOutput.split('\n')) {
        ...
        if (!Number.isFinite(pid) || pid <= 0 || !Number.isFinite(ppid))
            continue;
        const etime = parts[2];
...
export function parseEtimeSeconds(etime) {
    ...
    if (segments.some((n) => !Number.isFinite(n)))
        return 0;
    let h = 0;
    let m = 0;
    let s = 0;
 ...
 */
export function classifyVitestOrphans(processes, opts = {}) {
    ...
    for (const p of processes) {
        if (p.ageSeconds >= maxAgeSec)
            add(p);
        if (p.ppid === 1)
            add(p);
        else if (!alive.has(p.ppid))
            add(p);
    }
    return [...flagged.values()];
}
function defaultMaxCount() {
    const raw = process.env.FW_VITEST_ORPHAN_COUNT;
    if (raw && /^\d+$/.test(raw))
        return Math.max(1, Number(raw));
    return 3;
}
function defaultMaxAgeSec() {
    const raw = process.env.FW_VITEST_ORPHAN_AGE_SEC;
    if (raw && /^\d+$/.test(raw))
        return Math.max(60, Number(raw));
    return 600;
}
const ORPHAN_HINT_PREFIX = 'Reap with `flywheel_remediate({ checkName: "orphan_vitest_workers", mode: "execute", autoConfirm: true })` or kill PIDs manually.';
export async function checkOrphanVitestWorkers(exec, cwd, signal, timeout, now) {
    const start = now();
    if (signal.aborted) {
        return {
...
//# sourceMappingURL=orphan-vitest-workers.js.map