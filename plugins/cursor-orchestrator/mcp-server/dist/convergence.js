...
function diffLineCounts(prior, current) {
    ...
    let removed = 0;
    for (const c of aSet.values())
        removed += c;
    return { addedLines: added, removedLines: removed };
}
export function computeRevisionMetrics(currentMd, priorMd, opts = {}) {
    const revisionId = opts.revisionId ?? `rev-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
...
function detectOscillation(revs) {
    if (revs.length < 3)
        return { signFlips: 0, detected: false };
    let prevSign = 0;
    let flips = 0;
    for (let i = 1; i < revs.length; i++) {
...
function isMonotonicallyClimbing(scores) {
    if (scores.length < 2)
        return false;
 ...
 */
function computeScore(signals, revCount) {
    if (revCount < 2)
        return 0;
 ...
 */
export function appendRevision(state, metrics, planSlug, opts = {}) {
    if (state && state.scoreVersion !== SCORE_VERSION) {
        throw new Error(`appendRevision: state.scoreVersion=${state.scoreVersion} does not match SCORE_VERSION=${SCORE_VERSION}; caller must surface score_version_mismatch`);
    }
    const priorRevs = state?.revisions ?? [];
    const allRevs = [...priorRevs, metrics];
    const ringRevs = allRevs.length > REVISION_BUFFER_SIZE
 ...
 */
export function defaultStep545Action(state) {
    if (state.status === "oscillating")
        return "refine";
    if (state.status === "diverging")
        return "refine";
    if (state.score >= 0.9)
        return "approve";
    if (state.score >= 0.75)
 ...
 */
export function parseConvergenceState(raw) {
    let parsed;
    try {
