...
const RECOVER_PHASES = new Set([
    "implementing",
    "iterating",
    "reviewing",
]);
function capWarnings(warnings, cap) {
...
function isSuccessfulBeadResult(status) {
    return status === "success" || status === "closed";
}
export function extractCheckpointCandidateIds(state) {
    const ids = new Set();
...
export function classifyCheckpointTrust(envelope, opts = {}) {
    ...
    if (envelope.gitHead &&
        ...
        branchMismatch = true;
        warnings.push(`branch mismatch: checkpoint gitHead ${envelope.gitHead.slice(0, 7)} ≠ current ${opts.currentGitHead.slice(0, 7)}`);
    }
    ...
    if (opts.knownBeadIds && candidateIds.length > 0) {
        ...
        if (missing.length > 0) {
            warnings.push(`${missing.length} checkpoint candidate bead(s) missing from br list`);
        }
    }
    const trusted = warnings.length === 0 &&
...
async function readCurrentGitHead(cwd) {
    try {
        const { stdout } = await execFileAsync("git", ["rev-parse", "HEAD"], { cwd, timeout: RECOVER_GIT_HEAD_TIMEOUT_MS });
...
function capCandidateIds(ids, cap) {
    if (ids.length <= cap) {
        return { ids, truncated: false };
    }
    return { ids: ids.slice(0, cap), truncated: true };
}
export async function scanBeadCandidates(ctx, opts = {}) {
    ...
    try {
        ...
        const message = err instanceof Error ? err.message : "bead scan failed";
...
export function degradeToManual(warnings, caps) {
...
function mergeWarnings(base, extra, warningCap) {
    return capWarnings([...base, ...extra], warningCap);
}
function filterCandidatesToKnown(candidateIds, knownBeadIds) {
    if (!knownBeadIds) {
...
async function probeBeadsForTrust(ctx, timeoutMs) {
    try {
...
export async function resolveRecoveryContext(ctx, args = {}, caps) {
    ...
    const warnings = [];
    const [checkpointSettled, gitHeadSettled, beadProbeSettled] = await Promise.allSettled([
        probeCheckpoint(ctx.cwd),
        readCurrentGitHead(ctx.cwd),
        probeBeadsForTrust(ctx, resolvedCaps.beadScanTimeoutMs),
    ]);
    const currentGitHead = gitHeadSettled.status === "fulfilled" ? gitHeadSettled.value : undefined;
    ...
    if (checkpointSettled.status === "fulfilled" &&
        ...
        const { beadIds: filteredCandidates, warnings: filterWarnings } = filterCandidatesToKnown(rawCandidates, knownBeadIds);
    ...
    else if (checkpointSettled.status === "fulfilled" &&
        !checkpointSettled.value.ok) {
...
//# sourceMappingURL=recover-gates.js.map