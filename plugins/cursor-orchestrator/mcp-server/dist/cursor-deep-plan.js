...
export const DEFAULT_CURSOR_DEEP_PLAN_MODELS = {
    correctness: "opus-4.6",
    ergonomics: "composer-2.5",
    robustness: "gpt-5.5-xhigh",
    synthesis: "opus-4.6",
};
const ENV_BY_PERSPECTIVE = {
...
export function useClaudeDeepPlanBackend() {
...
function configOverride(cwd) {
    ...
    const out = {};
    if (dp.correctness)
        out.correctness = dp.correctness;
    if (dp.ergonomics)
        out.ergonomics = dp.ergonomics;
    if (dp.robustness)
        out.robustness = dp.robustness;
    if (dp.synthesis)
        out.synthesis = dp.synthesis;
    return Object.keys(out).length > 0 ? out : undefined;
}
/** Resolve deep-plan models: defaults → flywheel.config.yaml → env. */
export function getCursorDeepPlanModels(cwd) {
    const fromConfig = configOverride(cwd) ?? {};
    return {
        correctness: envOverride("correctness") ??
            fromConfig.correctness ??
            DEFAULT_CURSOR_DEEP_PLAN_MODELS.correctness,
        ergonomics: envOverride("ergonomics") ??
            fromConfig.ergonomics ??
            DEFAULT_CURSOR_DEEP_PLAN_MODELS.ergonomics,
        robustness: envOverride("robustness") ??
...
export function formatCursorDeepPlanModelTable(models) {
    return [
...
export function buildCursorDeepPlanInstructions(planAgents, synthesisModel, planSlug) {
    const spawnLines = planAgents.map((a) => `- **${a.name}** (\`${a.perspective}\`): \`Task({ model: "${a.model}", subagent_type: "generalPurpose", run_in_background: true, description: "...", prompt: <task from planAgents> })\` — tier ${a.tier}`);
    return [
...
export function buildCursorDeepPlanAgents(basePrompt, models) {
    return [
        {
...
//# sourceMappingURL=cursor-deep-plan.js.map