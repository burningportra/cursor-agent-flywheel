...
export function buildImplTickCoordinatorPlaybook(cfg, opts) {
...
function attachGateFields(snapshot) {
...
function supervisionSnapshot(baseSnapshot, cfg, mode, stuckBeadIds) {
    return {
...
function beadsForWaveReview(all, ids) {
...
export async function runImplTickCore(ctx, args) {
    ...
    if (pendingRange) {
        if (await verdictFileExists(cwd, pendingRange)) {
            ...
            const supervision = attachGateFields(supervisionSnapshot({ ...baseSnapshot, pendingBatchReviewRange: undefined }, cfg, 'monitor'));
        ...
        const inProgressGate = attachGateFields(supervisionSnapshot(baseSnapshot, cfg, 'batch_review_in_progress'));
    ...
    const batchReviewForced = args.forceBatchReview === true
        && threshold > 0
    ...
    if (shouldTriggerBatchReview(threshold, commitsSinceBaseline) || batchReviewForced) {
        ...
        const dispatchGate = autoBatchReview
            ? {}
            : attachGateFields(supervisionSnapshot({ ...baseSnapshot, pendingBatchReviewRange: shaRange }, cfg, 'batch_review_dispatch'));
        return buildTickResult(epochAtTickStart, state, epochGuards, dispatchLines.join('\n'), {
    ...
    if (closed.length > 0) {
        ...
        if (outcome?.nextStep?.kind === 'batch_review_due') {
            ...
            const verifyText = autoBatchReview
                ...
                : waveResult.content[0]?.text ?? 'Batch review due after wave verify.';
            return buildTickResult(epochAtTickStart, state, epochGuards, verifyText, {
                kind: 'batch_review_dispatch',
                tickAt,
                nextTickInSeconds: cfg.intervalSeconds,
        ...
        if (outcome?.waveComplete && outcome.nextStep?.kind === 'wrap_up_gate') {
            return buildTickResult(epochAtTickStart, state, epochGuards, waveResult.content[0]?.text ?? 'Queue drained — call flywheel_wrap_up_gate.', {
                kind: 'wrap_up_ready',
                tickAt,
                nextTickInSeconds: cfg.intervalSeconds,
                snapshot: baseSnapshot,
                ...
                nextActionHint: areNextActionHintsEnabled(cwd)
                    ? buildQueueDrainedHint(epochAtTickStart, baseSnapshot.commitsSinceBaseline, threshold)
                    : undefined,
            });
        }
        if (!autoBatchReview
    ...
    const monitorGate = attachGateFields(supervisionSnapshot(baseSnapshot, cfg, 'monitor', stuck.length > 0 ? stuck.map((b) => b.id) : undefined));
