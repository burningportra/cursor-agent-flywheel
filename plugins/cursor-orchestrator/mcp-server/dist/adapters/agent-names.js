...
export const AGENT_NAME_ADJECTIVES = Object.freeze([
    'Coral',
    'Silent',
    'Amber',
    'Bright',
...
export const AGENT_NAME_NOUNS = Object.freeze([
    'Dune',
    'Harbor',
    'Fjord',
    'Grove',
 ...
 */
export const AGENT_NAME_POOL_SIZE = AGENT_NAME_ADJECTIVES.length * AGENT_NAME_NOUNS.length;
/**
 ...
 */
export function pickAgentName(seed) {
    if (seed.length === 0) {
 ...
 */
export function allocateAgentNames(count, baseSeed) {
    if (count < 0)
        throw new Error('count must be >= 0');
    if (count > AGENT_NAME_POOL_SIZE) {
        throw new Error(`requested ${count} names but pool only holds ${AGENT_NAME_POOL_SIZE}`);
    }
    const seen = new Set();
    const out = [];
    let salt = 0;
    while (out.length < count) {
        ...
        if (salt > AGENT_NAME_POOL_SIZE * 4) {
            throw new Error(`failed to allocate ${count} unique names after ${salt} probes`);
        }
    }
    return out;
}
//# sourceMappingURL=agent-names.js.map