...
const beadGraphModulePath = isCompiled
    ? "../bead-graph.js"
    : "../src/bead-graph.js";
const { buildBeadGraph } = (await import(beadGraphModulePath));
...
function parseArgs(argv) {
    const args = { port: 0, noOpen: false };
    for (let i = 0; i < argv.length; i++) {
        ...
        if (a === "--port") {
            ...
            process.stdout.write("Usage: bead-viewer [--port N] [--no-open] [--db <path>]\n");
            process.exit(0);
        }
        else {
...
function makeRateLimiter() {
    ...
    return (ip) => {
        ...
        cur.count++;
        if (cur.count > RATE_LIMIT_PER_SEC)
            return false;
        return true;
    };
}
function execBr(args, cwd) {
    return new Promise((resolve, reject) => {
        child_process.execFile("br", args, {
...
async function handleGraph(res) {
...
async function handleBead(res, id) {
...
async function handleAsset(res, fileRel) {
...
async function main() {
    ...
    if (!LOOPBACK_HOSTS.has(bind)) {
        process.stderr.write(`bead-viewer: refused non-loopback bind FW_VIEWER_BIND=${bind}\n`);
        process.exit(2);
    }
    let activeConn = 0;
...
main().catch((e) => {
    process.stderr.write(`bead-viewer: fatal: ${e.message}\n`);
    process.exit(1);
});
//# sourceMappingURL=bead-viewer.js.map