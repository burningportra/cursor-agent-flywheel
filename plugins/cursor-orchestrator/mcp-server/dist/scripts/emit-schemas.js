...
async function main() {
    ...
    process.stdout.write(`emit-schemas: wrote ${entries.length} schema(s) → ${INPUTS_DIR}\n` +
        `             manifest sha256=${manifest.sha256}\n`);
}
main().catch((err) => {
    process.stderr.write(`emit-schemas FAILED: ${err.message ?? String(err)}\n`);
    process.exit(1);
});
