...
export function useCursorGraderBackend() {
    const b = process.env.FW_GRADER_BACKEND?.trim().toLowerCase();
    if (b === "codex" || b === "claude" || b === "claude-cli")
        return false;
    return true;
}
export function resolveCursorGraderModel(cwd) {
    const { config } = loadFlywheelConfigWithWarnings(cwd);
    const fromConfig = config.grader?.model?.trim();
    if (fromConfig)
        return fromConfig;
...
export function buildCursorGraderCoordinatorPlaybook(model) {
    return [
...
//# sourceMappingURL=cursor-grader.js.map