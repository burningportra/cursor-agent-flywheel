...
function rowFor(check) {
    ...
    const dur = typeof check.durationMs === 'number'
...
export function renderDoctorReport(report) {
    ...
    const overallTag = report.overall === 'green'
        ? color.green('OK')
    ...
    if (report.criticalFails > 0) {
        p.outro(color.red(`${report.criticalFails} critical check(s) failed — see hints above.`));
    }
