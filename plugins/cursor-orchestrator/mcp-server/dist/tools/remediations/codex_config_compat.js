...
import { parseCodexConfigTopLevelModel, isCodexIncompatibleModel, } from '../doctor.js';
...
function backupPathFor(target, tsIso) {
 ...
 */
export function commentOutTopLevelModel(content) {
...
export const codexConfigCompatHandler = {
    description: 'Comment out incompatible top-level model line in ~/.codex/config.toml (with timestamped backup)',
    mutating: true,
    reversible: true,
    async buildPlan(_ctx) {
        ...
        if (model === null) {
            return {
                description: 'No top-level `model = ...` override in ~/.codex/config.toml — already compatible.',
                steps: [],
                mutating: false,
                reversible: true,
            };
        ...
        return {
            description: `Back up ~/.codex/config.toml to ~/.codex/config.toml.bak.<timestamp>, then comment out the line setting model="${model}" so the codex-companion app-server falls through to its built-in default. Restore with \`mv\` from the .bak file.`,
            steps: [
    ...
    async execute(_ctx) {
        ...
        const { rewritten, changedLine, changedLineNumber } = commentOutTopLevelModel(content);
        if (changedLine === null) {
        ...
        return {
            stepsRun: 2,
            stdout: `commented out line ${changedLineNumber} (model="${model}") in ${target}; backup at ${backup}`,
        };
    },
    async verifyProbe(_ctx) {
        const target = codexConfigPath();
        if (!existsSync(target)) {
...
//# sourceMappingURL=codex_config_compat.js.map