...
function lookupEffort(id, version) {
    if (id === UNTEMPLATED_KEY)
        return { effort: null };
...
async function getStartedTs(beadId, exec, signal) {
    try {
        const res = await exec('git', ['log', `--grep=${beadId}`, '--format=%aI', '--reverse', '-1'], { timeout: GIT_TIMEOUT_MS, signal });
        if (res.code !== 0)
            return null;
...
export async function runCalibrate(args, exec, signal) {
    const { cwd, sinceDays } = args;
    let brResult;
    try {
        brResult = await exec('br', ['list', '--json', '--status', 'closed'], { timeout: BR_TIMEOUT_MS, cwd, signal });
    }
    catch (err) {
    ...
    for (const [groupKey, group] of groups) {
        if (group.templateKey.id === UNTEMPLATED_KEY)
            continue;
        const { effort, resolvedVersion } = lookupEffort(group.templateKey.id, group.templateKey.version);
        ...
        const proxyStartedCount = group.samples.reduce((acc, s) => acc + (s.proxyStarted ? 1 : 0), 0);
