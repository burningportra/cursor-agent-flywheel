...
import { buildCursorDeepPlanAgents, buildCursorDeepPlanInstructions, getCursorDeepPlanModels, useClaudeDeepPlanBackend, } from '../cursor-deep-plan.js';
import { buildCursorDuelRun, buildDuelModelsGate, formatCursorDuelModelTable, recommendDuelModels, useNtmDuelBackend, } from '../cursor-duel.js';
...
The synthesis must be BETTER than any individual plan, not a lowest-common-denominator average. Preserve bold ideas; don't sand them down.`;
    if (!useClaudeDeepPlanBackend()) {
        ...
        const instructions = buildCursorDeepPlanInstructions(planAgents, cursorModels.synthesis, planSlugForPaths);
        const payload = {
        ...
        return okResult(`${instructions}\n\n---\n\n**Goal:** ${goal}${constraintsSummary}\n\n**Synthesis prompt** is in \`data.synthesisPrompt\`.`, 'planning', payload);
    }
