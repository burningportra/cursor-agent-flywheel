...
import { AUTO_REVIEW_FINDING_LABEL, buildThermoNuclearPersonaTask, reviewVerdictPath, reviewVerdictRel, THERMO_PREAMBLE, THERMO_SUBAGENT_TYPE, } from '../combined-review-prompt.js';
...
export async function acceptWaveBeadsAtReview(ctx, beadIds) {
    ...
    for (const beadId of beadIds) {
        ...
        if (!result.ok) {
            saveState(state);
            return errorResult(state.phase, 'cli_failure', `Failed to close bead ${beadId} while accepting wave review.`, { beadId, partiallyClosed, stderr: result.stderr });
        }
...
export async function runReview(ctx, args) {
    ...
    if (args.action === 'looks-good') {
        ...
        if (!closeResult.ok) {
            return errorResult(state.phase, 'cli_failure', `Failed to close bead ${beadId} during review accept.`, { beadId, stderr: closeResult.stderr });
        }
    ...
    if (args.action === 'hit-me') {
        ...
        try {
            ...
            if (err.code !== 'ENOENT') {
                return errorResult(state.phase, 'internal_error', `Failed to check review verdict file: ${errMsg(err)}`, { verdictPath });
            }
            if (state.beadHitMeTriggered?.[beadId]) {
                return okResult(state.phase, `## Hit-me review in progress — ${beadId} (round ${round})\n\n` +
...
async function handleBatchReview(ctx, args) {
    ...
    return collectReviewVerdict(ctx, {
        ...
        needsAttentionMessage: `## Batch Review: NEEDS ATTENTION — ${shaRange}\n\n` +
...
async function runGates(ctx, action) {
    ...
    if (action === 'looks-good') {
        ...
        if (consecutiveClean >= 2) {
            state.phase = 'iterating';
            saveState(state);
            return okResult('iterating', `## Review gates complete (${consecutiveClean} clean rounds)
