...
export async function runProfile(ctx, args) {
    ...
    if (hasBeads) {
        ...
        if (brListResult.code === 0) {
            try {
                const beads = JSON.parse(brListResult.stdout);
                const open = beads.filter((b) => b.status === 'open' || b.status === 'in_progress');
                const deferred = beads.filter((b) => b.status === 'deferred');
...
// ─── Repo scanning ────────────────────────────────────────────
async function buildRepoProfile(exec, cwd) {
    const profile = {
        name: '',
        languages: [],
        frameworks: [],
    ...
    // Recent commits
    const gitLogResult = await exec('git', ['log', '--oneline', '--format=%H|%s|%ai|%an', '-20'], { cwd, timeout: 10000 });
    ...
    // File structure
    const findResult = await exec('find', ['.', '-maxdepth', '3', '-not', '-path', './.git/*', '-not', '-path', './node_modules/*', '-not', '-path', './.claude-orchestrator/*'], { cwd, timeout: 10000 });
    ...
    for (const line of (findResult.stdout || '').split('\n')) {
        ...
        if (match)
            extensions[match[1]] = (extensions[match[1]] || 0) + 1;
    }
    const langMap = {
    ...
    if (profile.keyFiles['package.json']) {
        ...
        if (pkg) {
            ...
            if (deps['next'])
                profile.frameworks.push('Next.js');
            if (deps['react'])
                profile.frameworks.push('React');
            if (deps['vue'])
                profile.frameworks.push('Vue');
            if (deps['express'])
                profile.frameworks.push('Express');
            if (deps['fastify'])
                profile.frameworks.push('Fastify');
    ...
    // TODOs
    const todoResult = await exec('grep', ['-rn', '--include=*.ts', '--include=*.js', '--include=*.py', '--include=*.go', '--include=*.rs',
        '-E', 'TODO|FIXME|HACK|XXX', '.', '--exclude-dir=node_modules', '--exclude-dir=.git'], { cwd, timeout: 10000 });
