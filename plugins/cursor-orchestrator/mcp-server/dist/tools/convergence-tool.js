...
import { parseConvergenceState, ConvergenceStateSchema, SCORE_VERSION, } from '../convergence.js';
...
export function planSlugFromIdentifier(identifierOrPath) {
...
export function convergencePath(cwd, planSlug) {
...
export async function readConvergenceFromDisk(cwd, planSlug) {
 ...
 */
export async function writeConvergenceToDisk(cwd, state) {
    ...
    if (state.scoreVersion !== SCORE_VERSION) {
        throw new Error(`writeConvergenceToDisk: refusing to write state with scoreVersion=${state.scoreVersion} (current=${SCORE_VERSION})`);
    }
...
export async function runConvergence(ctx, args) {
    const planSlugRaw = args.planSlug;
...
//# sourceMappingURL=convergence-tool.js.map