 ...
 */
export function normalizeWatchPath(pathInput, cwd) {
...
export function resolveDefaultWatchPaths(state, cwd) {
    const candidates = [];
    if (state.planDocument)
        candidates.push(state.planDocument);
    if (state.outcomeRubricPath)
        candidates.push(state.outcomeRubricPath);
    ...
    for (const raw of candidates) {
        ...
        seen.add(norm);
        out.push(norm);
        if (out.length >= MAX_WATCH_FILES)
            break;
    }
    return out;
}
function hashWatchEntry(cwd, relPath) {
 ...
 */
export function registerProfileWatch(state, cwd, paths, opts) {
 ...
 */
export function checkProfileStaleness(cwd, state, config, opts) {
...
export function clearProfileStaleFlags(state) {
    return {
        ...state,
        profileStale: false,
        profileStaleReason: undefined,
