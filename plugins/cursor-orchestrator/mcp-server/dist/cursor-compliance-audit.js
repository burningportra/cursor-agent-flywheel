...
export function useCursorComplianceBackend() {
    const b = process.env.FW_COMPLIANCE_BACKEND?.trim().toLowerCase();
    if (b === 'claude' || b === 'claude-cli')
        return false;
    return true;
}
export function resolveCursorComplianceModel(cwd) {
    const { config } = loadFlywheelConfigWithWarnings(cwd);
    const fromConfig = config.compliance?.model?.trim();
    if (fromConfig)
        return fromConfig;
...
export function buildComplianceCoordinatorPlaybook(model) {
    return [
...
export function parseComplianceOverride(raw) {
    const trimmed = raw.trim();
    if (!trimmed)
        return { skipAll: false, beadIds: new Set() };
...
//# sourceMappingURL=cursor-compliance-audit.js.map