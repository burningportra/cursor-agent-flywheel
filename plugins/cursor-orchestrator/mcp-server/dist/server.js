...
const server = new Server({ name: "claude-orchestrator", version: "2.0.0" }, { capabilities: { tools: {} } });
// ─── Tool definitions ─────────────────────────────────────────
const TOOLS = [
    {
        name: "orch_profile",
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const ctx = {
        ...
        cwd,
        state,
        saveState: (s) => saveState(cwd, s),
        clearState: () => clearState(cwd),
    };
    try {
        switch (name) {
            case "orch_profile":
                return await runProfile(ctx, args);
            case "orch_discover":
                return await runDiscover(ctx, args);
            case "orch_select":
                return await runSelect(ctx, args);
            case "orch_plan":
                return await runPlan(ctx, args);
            case "orch_approve_beads":
                return await runApprove(ctx, args);
            case "orch_review":
                return await runReview(ctx, args);
            case "orch_memory":
                return await runMemory(ctx, args);
            default:
                return {
