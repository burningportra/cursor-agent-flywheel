...
import { WAVE_REVIEW_CONFIRM_ACTIONS, WRAP_UP_CONFIRM_ACTIONS, } from './types.js';
...
const PRIMARY_TOOLS = [
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                skipEnv: {
                    type: 'string',
                    description: 'Override list: "1"/"true" skips entire audit; comma-separated bead ids skip only those beads',
                },
                afterTask: {
                    type: 'boolean',
                    description: 'Cursor port: re-call after compliance Task completes — reads latest pass dir without spawning claude',
                },
            },
            required: ['cwd', 'beadIds'],
        },
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                confirmImplModels: {
                    description: 'After implModelsGate, pass user choice: "recommended" (option 1), "defaults", { uniform: "<slug>" }, or { simple, medium, complex }. Re-use the same closedBeadIds.',
                },
                skipImplModelsGate: {
                    type: 'boolean',
    ...
    },
    {
        ...
        description: 'MANDATORY after a wave of impl agents finishes: returns userGate + askQuestion for Cursor AskQuestion (clickable UI). Call AskQuestion with askQuestion — do not ask user to type 1/2/3 in prose.',
        inputSchema: {
            type: 'object',
            properties: {
                cwd: { type: 'string', description: 'Project working directory (absolute path)' },
                ...
                confirmAction: {
                    ...
                    description: 'User AskQuestion selection — records steering and bumps coordinator epoch (E8). looks-good-all closes beads; fresh-eyes/self-review dispatch review (pass reviewBeadId for multi-bead waves). Re-call after user picks.',
                },
                reviewBeadId: {
                    type: 'string',
                    description: 'Bead to review when confirmAction is fresh-eyes or self-review and beadIds has more than one entry.',
                },
            },
            required: ['cwd', 'beadIds'],
        },
    },
    {
        ...
        description: 'MANDATORY when all beads are reviewed and the queue is empty: wrap-up userGate + askQuestion. Call AskQuestion with askQuestion before any commit. confirmWrapUp after user clicks.',
        inputSchema: {
            type: 'object',
            properties: {
                cwd: { type: 'string', description: 'Project working directory (absolute path)' },
    ...
    },
    {
        ...
        description: 'MANDATORY after creating beads (Step 5.5–6): returns askQuestion for review, score, polish, coverage, dedup, and launch menus. Call AskQuestion with askQuestion — do not use AskUserQuestion prose. step=review first; on Start pick step=launch before flywheel_approve_beads action=start.',
        inputSchema: {
            type: 'object',
            properties: {
                cwd: { type: 'string', description: 'Project working directory (absolute path)' },
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                confirmDuelModels: {
                    description: 'User choice: "recommended", "defaults", or { wizard_a, wizard_b, wizard_c? }',
                },
                skipDuelModelsGate: {
                    type: 'boolean',
    ...
    },
    {
        ...
        description: 'One-time gate: show default implement models (flywheel.config.yaml implement:) and persist the user choice before spawning Cursor Task impl agents. Also persists commitBatchThreshold from pre-flight (or config/env default). Call without confirmImplModels to get implModelsGate; call again with confirmImplModels after the user replies.',
        inputSchema: {
            type: 'object',
            properties: {
                cwd: { type: 'string', description: 'Project working directory (absolute path)' },
                confirmImplModels: {
                    description: 'User choice: "defaults", { uniform: "<slug>" }, or { simple, medium, complex }.',
                },
                commitBatchThreshold: {
                    type: 'integer',
                    minimum: 0,
                    description: 'Commit-batch fresh-eyes cadence (0 = off). On confirm, persists to checkpoint; when omitted, uses flywheel.config.yaml / FW_COMMIT_BATCH_THRESHOLD if set.',
                },
            },
            required: ['cwd'],
        },
    ...
    },
    {
        ...
        description: 'Cursor implementation supervision tick (~4 min cadence): snapshot beads/commits, trigger commit-batch fresh-eyes (Task + verdict file), advance waves when closedBeadIds provided, and return impl Task specs. Re-call on interval until queue drains. Set state.commitBatchThreshold at impl pre-flight (or FW_COMMIT_BATCH_THRESHOLD).',
        inputSchema: {
            type: 'object',
            properties: {
                cwd: { type: 'string', description: 'Project working directory (absolute path)' },
                ...
                commitBatchThreshold: {
                    type: 'integer',
                    minimum: 0,
                    description: 'Persist commit-batch fresh-eyes threshold for this session (0 = disable). Also readable from flywheel.config.yaml impl_tick.commit_batch_threshold or FW_COMMIT_BATCH_THRESHOLD.',
                },
                forceBatchReview: {
                    type: 'boolean',
                    description: 'Trigger commit-batch fresh-eyes review even when below threshold (requires commits since baseline > 0).',
                },
            },
            required: ['cwd'],
        },
    ...
    },
    {
        ...
        description: 'Step 0d start menu for Cursor: returns askQuestion payload, routeHints, and primary entry points for the detected variant. Call after flywheel_observe.',
        inputSchema: {
            type: 'object',
            properties: {
                cwd: { type: 'string', description: 'Project working directory (absolute path)' },
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                graderStdout: {
                    type: 'string',
                    description: 'Cursor port: JSON stdout from the decorrelated grader Task (second call after grader_deferred).',
                },
            },
            required: ['cwd'],
        },
...
const DEFAULT_RUNNERS = {
    ...
    flywheel_confirm_impl_models: runConfirmImplModels,
    flywheel_duel: runDuel,
    flywheel_wave_review_gate: runWaveReviewGate,
    flywheel_wrap_up_gate: runWrapUpGate,
    flywheel_bead_approval_gate: runBeadApprovalGate,
    flywheel_memory: runMemory,
    flywheel_doctor: runDoctor,
    flywheel_get_skill: runGetSkill,
    flywheel_observe: runObserve,
    flywheel_impl_tick: runImplTick,
    flywheel_start_menu: runStartMenu,
...
function zodIssueToValidationError(toolName, issue) {
...
function validateGateToolArgsWithZod(toolName, args) {
