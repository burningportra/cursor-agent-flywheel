...
interface SynthModule {
  synthesizePlans: (plans: DeepPlanResult[], opts?: { whole?: boolean }) => Promise<string>;
  shouldUseSectionWise: (repoFileCount: number) => boolean;
}

interface DeepPlanResult {
  name: string;
  model: string;
  plan: string;
  exitCode: number;
...
const { synthesizePlans, shouldUseSectionWise } = (await import(synthModulePath)) as SynthModule;

const __dirname = dirname(fileURLToPath(import.meta.url));
const FIXTURE_DIR = join(__dirname, "fixtures", "deep-plan-bench");
const FIXTURE_NAMES = ["correctness", "ergonomics", "robustness"] as const;

function loadFixtures(): DeepPlanResult[] {
  ...
  if (shouldUseSectionWise(repoFiles)) {
    if (speedup < 25) {
      process.stderr.write(
        `[bench-deep-plan] FAIL: section mode only ${speedup.toFixed(1)}% faster, need >=25%\n`
      );
      process.exit(1);
    }
    ...
    process.stdout.write(
      `[bench-deep-plan] NOTE: bench corpus does not clear the ${500}-file threshold ` +
