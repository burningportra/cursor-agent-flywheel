    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        switch (a) {
            ...
            case "--profile": {
                const v = takeNext();
                if (v === null)
                    return { opts, error: "--profile requires full|cursor-native|cursor" };
...
function isExcludedByProfile(profile, skillDir) {
    if (normalizeProfile(profile) === "full")
        return false;
    return CURSOR_PROFILE_EXCLUDE.has(skillDir);
}
/** Prefer `_foo.cursor.md` over `_foo.md` when both exist (cursor-native profile). */
function resolveStartSubFiles(startEntries, profile) {
    ...
    for (const [stem, files] of byStem) {
        if (prof === "cursor-native" && CURSOR_NATIVE_START_EXCLUDE.has(stem))
            continue;
...
async function walkSources(sourceRoot, profile) {
    ...
    for (const entry of entries) {
        ...
        if (!st.isDirectory())
            continue;
        if (isExcludedByProfile(profile, entry))
            continue;
...
export async function build(opts) {
    ...
    const generatedAt = process.env.FW_BUNDLE_GENERATED_AT ??
...
export async function main(argv) {
    ...
    try {
        ...
        process.stderr.write(`bundled ${result.bundle.entries.length} entries (profile=${opts.profile}) → ${result.outputPath} ` +
