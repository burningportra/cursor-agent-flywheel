...
function parseArgs(argv: string[]): { opts: CliOpts; error: string | null } {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case "--bundle": {
        const v = takeNext();
        if (v === null) return { opts, error: "--bundle requires a path argument" };
        opts.bundle = v;
        break;
      }
      case "--source-root": {
...
export async function main(argv: string[]): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(
      `check-skills-bundle: cannot read bundle ${bundlePath}: ${String(err)}\n`,
    );
    return 4;
  }
  ...
  let bundle: Bundle;
  try {
    bundle = JSON.parse(bundleRaw) as Bundle;
  } catch (err) {
    process.stderr.write(
      `check-skills-bundle: invalid JSON at ${bundlePath}: ${String(err)}\n`,
    );
    return 4;
  }

  const drifts: DriftLine[] = [];
  ...
  const bundleByName = new Map<string, BundleEntry>();
  for (const e of bundle.entries) bundleByName.set(e.name, e);
