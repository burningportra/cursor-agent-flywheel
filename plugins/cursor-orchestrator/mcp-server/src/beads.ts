...
const BEAD_FILE_PATH_RE =
  /[a-zA-Z0-9_\-./]+(?:\.ts|\.tsx|\.js|\.jsx|\.json|\.md|\.py|\.rs|\.go|\.yaml|\.yml|\.toml)/g;

function normalizeBeadFilePath(raw: string): string {
 ...
 */
export function collectBeadFilePaths(bead: Bead): string[] {
...
export function countAcceptanceCriteria(description: string): number {
...
export function parseBeadEffort(bead: Bead): "S" | "M" | "L" | "XL" | undefined {
  const desc = bead.description ?? "";
  const fromDesc = desc.match(EFFORT_IN_DESC_RE);
  if (fromDesc?.[1]) {
    return fromDesc[1].toUpperCase() as "S" | "M" | "L" | "XL";
  }
  if (typeof bead.estimate === "number" && bead.estimate > 0) {
    if (bead.estimate <= 45) return "S";
    if (bead.estimate <= 120) return "M";
    if (bead.estimate <= 300) return "L";
    return "XL";
  }
  return undefined;
}
