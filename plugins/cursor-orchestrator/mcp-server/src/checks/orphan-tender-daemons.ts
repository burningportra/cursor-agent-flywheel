...
export const ORPHAN_TENDER_DAEMONS_CHECK_NAME = 'orphan_tender_daemons' as const;
 ...
 * tmux teardown).
 */
export function parseTenderDaemonProcesses(psOutput: string): TenderDaemonProcess[] {
  const out: TenderDaemonProcess[] = [];
  for (const line of psOutput.split('\n')) {
    const trimmed = line.trim();
    if (trimmed.length === 0) continue;
    if (!trimmed.includes('tender-daemon')) continue;
 ...
 */
export function parseTmuxSessions(tmuxOutput: string): Set<string> {
...
const ORPHAN_HINT_PREFIX =
  'Stop them with `kill -TERM <pid>` (one PID per daemon). Re-run flywheel_doctor afterwards to confirm.';

/**
 * Doctor check implementation. Never throws — returns a DoctorCheck row.
 */
export async function checkOrphanTenderDaemons(
  exec: ExecFn,
  cwd: string,
  signal: AbortSignal,
  timeout: number,
  ...
  try {
    ...
    if (tmux.code === 0) liveSessions = parseTmuxSessions(tmux.stdout);
  } catch {
