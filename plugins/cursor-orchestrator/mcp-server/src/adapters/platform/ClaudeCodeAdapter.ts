...
const CLAUDE_WORKTREE_ROOTS: readonly WorktreeScanRoot[] = [
  ...
  validateHooks(pluginRoot: string | null): readonly DiagnosticResult[] {
    ...
    if (!pluginRoot) {
      out.push({
        ...
        severity: 'yellow',
        message:
          '`CLAUDE_PLUGIN_ROOT` is not set — running outside Claude Code or without `/plugin install`',
    ...
    try {
      ...
      out.push({
        ...
        severity: 'red',
        message: `settings.json is not valid JSON: ${(err as Error).message}`,
