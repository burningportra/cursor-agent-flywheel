...
import {
  buildCursorGraderCoordinatorPlaybook,
  resolveCursorGraderModel,
  useCursorGraderBackend,
} from './cursor-grader.js';
...
export function resolveMaxOutcomeIterationsFromEnv(): number | undefined {
...
export function isGradeSkipped(
  result: GraderVerdict | GradeSkippedSentinel | GraderDeferredSentinel,
...
export function isGraderDeferred(
  result: GraderVerdict | GradeSkippedSentinel | GraderDeferredSentinel,
): result is GraderDeferredSentinel {
...
export async function gradeOutcome(
...
): Promise<GraderVerdict | GradeSkippedSentinel | GraderDeferredSentinel> {
  const { exec, cwd, state, saveState, signal } = ctx;
  ...
  try {
    ...
    const cursorDefer =
      useCursorGraderBackend() && !opts.grader && !args.graderStdout?.trim();
    if (cursorDefer) {
      return {
        status: 'deferred',
        kind: 'cursor_grader_task',
        ...
        instructions:
          'Spawn one Cursor Task with the prompt, then flywheel_grade_outcome({ cwd, graderStdout }) with the Task JSON stdout.',
      };
    }

    // Decide preferred grader (codex when configured, else claude). Cursor submit uses graderStdout.
    const preferModel: 'codex' | 'claude' =
      process.env.FW_GRADER_FORCE_CLAUDE === '1' ? 'claude' : 'codex';
    const timeoutMs = Number(process.env.FW_GRADER_TIMEOUT_MS ?? GRADER_TIMEOUT_MS_DEFAULT);

    const startWall = now();
    let stdout: string;
    let modelUsed: 'codex' | 'claude' | 'cursor';
