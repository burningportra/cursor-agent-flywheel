...
function questionLabel(q: AuqQuestion): string {
  if (q.header && q.header.length > 0) return q.header;
  if (q.question) return q.question.slice(0, 40);
  return "(unnamed)";
}

function optionLabel(opt: AuqOption): string {
  return opt.label ?? "(unlabeled)";
}

function asParsed(doc: Document): ParsedDocument {
  return doc as ParsedDocument;
}

export const auq001: Rule = {
  id: "AUQ001",
...
export const auqRules: Rule[] = [auq001, auq002, auq003, auq004];
