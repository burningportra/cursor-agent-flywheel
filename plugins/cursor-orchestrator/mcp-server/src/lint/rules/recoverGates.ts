...
const RECOVER_COMMAND_RE =
  /(?:^|[/\\])(?:recover-gates|flywheel-recover-gates|orchestrate-recover-gates)\.md$/i;

/** Prose gate prompts that must use AskQuestion + MCP gates instead. */
export const RECOV001_PHRASES: ReadonlyArray<string | RegExp> = [
  "want to commit",
  "should I commit",
  "should I continue",
...
const RECOV002_PATTERNS: ReadonlyArray<{ re: RegExp; label: string }> = [
  {
...
function isRecoverCommandFile(filePath: string): boolean {
  return RECOVER_COMMAND_RE.test(filePath.replace(/\\/g, "/"));
}

function findSectionSpans(source: string, headingRe: RegExp): SectionSpan[] {
  ...
  if (inSection) spans.push({ startLine, endLine: lines.length });
  return spans;
}

function inSection(line: number, spans: SectionSpan[]): boolean {
  return spans.some((s) => line >= s.startLine && line <= s.endLine);
}

function insideBacktickQuote(lineText: string, index: number): boolean {
  let backticks = 0;
...
function isNegativeInstruction(line: string): boolean {
  if (/\b(Never|Don't|don't|not ad-hoc|forbid)\b/i.test(line)) return true;
  if (/\bDo\s+\*\*not\*\*/i.test(line)) return true;
  if (/\bDo not\b/i.test(line)) return true;
  return false;
}
...
function compilePhrasePatterns(): RegExp[] {
  return RECOV001_PHRASES.map((p) => {
    if (p instanceof RegExp) return new RegExp(p.source, p.flags);
    return new RegExp(p.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi");
  });
}

function scanRecov001(source: string, filePath: string): Finding[] {
  ...
  for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
    ...
    if (inSection(line1, antiPatternSpans)) continue;
    if (isFallbackLine(lineText)) continue;
    if (isNegativeInstruction(lineText)) continue;
...
function scanRecov002(source: string, filePath: string): Finding[] {
  const findings: Finding[] = [];
  const lines = source.split("\n");
  const antiPatternSpans = findSectionSpans(source, /^##\s+Anti-patterns\b/i);
  ...
  for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
    ...
    const lineText = lines[lineIdx]!;
    if (inSection(line1, antiPatternSpans)) continue;
    if (inSection(line1, contextBudgetSpans)) continue;
    if (isNegativeInstruction(lineText)) continue;
    if (/\*\*not\*\*\s+`agent-flywheel:start`/i.test(lineText)) continue;
    ...
    for (const { re, label } of RECOV002_PATTERNS) {
      ...
      if (m === null) continue;
      if (insideBacktickQuote(lineText, m.index) && label.startsWith("load ")) continue;
      findings.push({
        ruleId: "RECOV002",
        severity: "error",
        file: filePath,
...
export const recov001: Rule = {
  id: "RECOV001",
  description:
    "Recovery command files must not instruct agents to ask prose commit/continue prompts.",
  severity: "error",
  check(doc: Document, ctx: RuleContext): Finding[] {
    if (!isRecoverCommandFile(ctx.filePath)) return [];
    return scanRecov001(doc.source ?? ctx.source, ctx.filePath);
  },
};

export const recov002: Rule = {
  id: "RECOV002",
  description:
    "Recovery command files must not instruct loading start_ceremony, start_discover, or start body.",
  severity: "error",
  check(doc: Document, ctx: RuleContext): Finding[] {
    if (!isRecoverCommandFile(ctx.filePath)) return [];
...
export const recoverGatesRules: Rule[] = [recov001, recov002];
