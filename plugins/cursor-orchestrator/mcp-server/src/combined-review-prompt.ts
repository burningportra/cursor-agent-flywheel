...
function buildStructuredFindingsBlock(shaRange: string): string {
 ...
 */
export function buildCombinedReviewPrompt(opts: CombinedReviewPromptOpts): string {
  const {
    round,
    memoryContext,
    allArtifacts,
...
export function reviewVerdictRel(beadId: string, round: number): string {
  return path.join('.pi-flywheel', 'review-verdicts', `${beadId}-r${round}.json`);
}

export function reviewVerdictPath(cwd: string, beadId: string, round: number): string {
