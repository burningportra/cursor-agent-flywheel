...
function makeExecCalls(beads: Bead[], readyBeads?: Bead[]): ExecCall[] {
  return [
    {
      cmd: 'br',
      args: ['list', '--json'],
...
function makeCtx(beads: Bead[], stateOverrides: Partial<FlywheelState> = {}) {
...
describe('I5 — approve_beads hotspot injection', () => {
  ...
  it('3 beads sharing server.ts via ### Files: section → 4-option menu + matrix', async () => {
    ...
    const sc = result.structuredContent as { data: { matrix?: HotspotMatrix; launchMode?: string }; nextStep?: { type: string; options?: Array<{ id: string }> } };
  ...
  it('single bead → legacy nextStep + matrix present with low/empty rows', async () => {
    ...
    const sc = result.structuredContent as { data: { matrix?: HotspotMatrix; launchMode?: string }; nextStep?: { type: string } };
    expect(sc.data.matrix).toBeDefined();
    const matrix = sc.data.matrix!;
  ...
  it('two beads sharing a file via prose only → 4-option menu (med severity)', async () => {
    ...
    const sc = result.structuredContent as { data: { matrix?: HotspotMatrix }; nextStep?: { type: string } };
    expect(sc.data.matrix).toBeDefined();
  ...
  it('GATE 1 REGRESSION: bead.description is mapped to HotspotInputBead.body', async () => {
    ...
    const { ctx } = makeCtx(beads);
    const result = await runApprove(ctx, { cwd: '/fake/cwd', action: 'start' });
    const sc = result.structuredContent as { data: { matrix?: HotspotMatrix } };
  ...
  it('matrix is idempotent across repeat approve_beads calls (determinism via I3)', async () => {
    ...
    const m1 = (r1.structuredContent as { data: { matrix: HotspotMatrix } }).data.matrix;
    const m2 = (r2.structuredContent as { data: { matrix: HotspotMatrix } }).data.matrix;
  ...
  it('empty bead list case is never reached — approve short-circuits before matrix compute', async () => {
    ...
    const result = await runApprove(ctx, { cwd: '/fake/cwd', action: 'start' });
    const sc = result.structuredContent as { data: { kind: string; matrix?: HotspotMatrix } };
