...
type CliCheckName = 'br_binary' | 'bv_binary' | 'ntm_binary' | 'cm_binary';
const CLI_CHECKS: CliCheckName[] = ['br_binary', 'bv_binary', 'ntm_binary', 'cm_binary'];

const EXPECTED_BINARY: Record<CliCheckName, string> = {
  br_binary: 'br',
  bv_binary: 'bv',
  ntm_binary: 'ntm',
  cm_binary: 'cm',
    ...
    describe(name, () => {
      ...
      it('plan steps reference the binary install command', async () => {
        ...
        try {
          ...
          const result = await runRemediate(
            { cwd, checkName: name, autoConfirm: false, mode: 'dry_run' },
            exec,
            new AbortController().signal,
          );
          expect('check' in result).toBe(true);
          const ok = result as { plan: { steps: string[]; mutating: boolean } };
      ...
      it('execute without autoConfirm is rejected with remediation_requires_confirm', async () => {
        ...
        try {
          const exec: ExecFn = async () => ({ code: 0, stdout: '', stderr: '' });
          const result = await runRemediate(
            { cwd, checkName: name, autoConfirm: false, mode: 'execute' },
            exec,
            new AbortController().signal,
          );
      ...
      it('execute with autoConfirm shells out via `bash -lc` and re-probes the binary', async () => {
        ...
        try {
          ...
          const result = await runRemediate(
            { cwd, checkName: name, autoConfirm: true, mode: 'execute' },
            exec,
            new AbortController().signal,
          );

          // Did NOT error.
          expect('isError' in result && (result as { isError?: boolean }).isError).not.toBe(true);
          const ok = result as {
            executed: boolean;
            stepsRun: number;
      ...
      it('verifyProbe returns false when the binary is still missing after install', async () => {
        ...
        try {
          ...
          const ok = result as { executed: boolean; verifiedGreen: boolean };
          expect(ok.executed).toBe(true);
