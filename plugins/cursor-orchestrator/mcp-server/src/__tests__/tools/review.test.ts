...
function brReadBeadsListCall(beads: Bead[]): ExecCall {
  return {
    cmd: 'br',
    args: [
      'list',
...
describe('runReview', () => {
  ...
  describe('acceptWaveBeadsAtReview (two-phase validate → bump → close)', () => {
    ...
    it('returns not_found with missing ids before epoch bump', async () => {
      ...
      expect(result.isError).toBe(true);
      const err = (result.structuredContent as { data: { error: { code: string; details: { missing: string[] } } } }).data.error;
    ...
    it('returns cli_failure with partiallyClosed on mid-loop close failure', async () => {
      ...
      expect(result.isError).toBe(true);
      const err = (result.structuredContent as { data: { error: { code: string; details: { partiallyClosed: string[]; beadId: string } } } }).data.error;
