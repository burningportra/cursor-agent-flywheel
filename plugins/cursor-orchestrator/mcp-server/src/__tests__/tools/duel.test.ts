...
describe('flywheel_duel', () => {
  it('returns duelModelsGate when models not confirmed', async () => {
    ...
    expect((data.duelModelsGate as { kind: string }).kind).toBe('confirm_duel_models');
  });
  ...
  it('returns cursorDuel spawn payload after confirm', async () => {
    ...
    expect((data.cursorDuel as { kind: string }).kind).toBe('cursor_duel_spawn');
