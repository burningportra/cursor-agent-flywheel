...
import {
  codexConfigCompatHandler,
  commentOutTopLevelModel,
} from '../../tools/remediations/codex_config_compat.js';
import { parseCodexConfigTopLevelModel } from '../../tools/doctor.js';
import type { HandlerCtx } from '../../tools/remediate.js';
import type { ExecFn } from '../../exec.js';

const noopExec: ExecFn = async () => ({ code: 0, stdout: '', stderr: '' });

function makeCtx(cwd: string): HandlerCtx {
...
describe('commentOutTopLevelModel (pure helper)', () => {
  it('comments a bare top-level model line', () => {
    const { rewritten, changedLine, changedLineNumber } = commentOutTopLevelModel(
      'model = "gpt-5.5"\n',
    );
  ...
  it('preserves leading whitespace on the commented line', () => {
    const { rewritten, changedLine } = commentOutTopLevelModel(
      '  model = "gpt-5"\n',
    );
...
describe('codexConfigCompatHandler.buildPlan', () => {
  let fake: ReturnType<typeof withFakeHome>;
...
describe('codexConfigCompatHandler.execute', () => {
  ...
  it('rewrites the config and writes a timestamped backup', async () => {
    ...
    const bakFiles = readdirSync(codexDir).filter((f) =>
      f.startsWith('config.toml.bak.'),
    );
