...
describe('impl_tick auto flags', () => {
  ...
  afterEach(() => {
    rmSync(tmp, { recursive: true, force: true });
    if (origAutoBatch !== undefined) process.env.FW_AUTO_BATCH_REVIEW = origAutoBatch;
    else delete process.env.FW_AUTO_BATCH_REVIEW;
    if (origAutoLoop !== undefined) process.env.FW_IMPL_TICK_AUTO_LOOP = origAutoLoop;
    else delete process.env.FW_IMPL_TICK_AUTO_LOOP;
  });
