...
describe("parseSkill", () => {
  ...
  it("throws on missing frontmatter", () => {
    expect(() => parseSkill("bad", "no frontmatter here")).toThrow(
      /no YAML frontmatter/,
    );
  });
...
describe("renderCodexSkillFile", () => {
  it("emits heading, description quote, and tools section", () => {
    ...
    expect(out).toContain(
      "- `codex-skill-invoke`  (equivalent: Claude `Skill`",
    );
...
describe("emitCodex (filesystem)", () => {
  ...
  beforeAll(async () => {
    ...
    await fs.writeFile(
      join(skillsRoot, "alpha-skill", "SKILL.md"),
      SKILL_NO_TOOLS,
      "utf8",
    );
    await fs.mkdir(join(skillsRoot, "beta-skill"), { recursive: true });
    await fs.writeFile(
      join(skillsRoot, "beta-skill", "SKILL.md"),
      SKILL_BLOCK_TOOLS,
      "utf8",
    );
    ...
    await fs.writeFile(
      join(skillsRoot, "_template", "SKILL.md"),
      SKILL_NO_TOOLS,
      "utf8",
    );
  ...
  it("writes AGENTS.md and per-skill files, skips templates and bad dirs", async () => {
    ...
    const beta = await fs.readFile(
      join(targetDir, ".codex", "skills", "beta-skill.md"),
      "utf8",
    );
