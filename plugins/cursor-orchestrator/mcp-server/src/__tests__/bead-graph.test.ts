...
import type { BeadGraph } from '../bead-graph.js';

// generatedAt is non-deterministic ISO timestamp; strip for snapshots.
function stripGeneratedAt(g: BeadGraph): Omit<BeadGraph, 'generatedAt'> {
...
describe('buildBeadGraph — disjoint subgraphs', () => {
  it('renders two disconnected components with no cross edges', () => {
    ...
    const crosses = g.edges.filter(
      (e) =>
        (['A', 'B'].includes(e.from) && ['C', 'D'].includes(e.to)) ||
