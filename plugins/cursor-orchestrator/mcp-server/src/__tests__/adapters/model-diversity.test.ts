...
function makeWhichExec(present: Record<string, string | false>): ExecFn {
  return async (cmd, args) => {
    ...
    if (path) return { code: 0, stdout: `${path}\n`, stderr: '' };
...
function caps(
  ...
  const mk = (
    p: 'claude' | 'codex' | 'gemini',
    a: boolean,
  ): CapabilitiesMap[typeof p] =>
    a
...
describe('splitBeadsByProvider', () => {
  ...
  it('missing codex → fallback redistributes share to claude/gemini + warning', () => {
    const plan = splitBeadsByProvider(
      ['b1', 'b2', 'b3'],
      caps({ codex: false }),
    );
  ...
  it('all CLIs missing → empty lanes + degraded + actionable warning', () => {
    const plan = splitBeadsByProvider(
      ['b1'],
      caps({ claude: false, codex: false, gemini: false }),
    );
