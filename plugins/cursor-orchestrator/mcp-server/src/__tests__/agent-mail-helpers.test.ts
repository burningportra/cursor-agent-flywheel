...
import {
  reserveOrFail,
  releaseReservations,
  type ReserveOrFailOpts,
  type ReleaseReservationsOpts,
} from "../agent-mail-helpers.js";
import type { ExecFn } from "../exec.js";

// ─── Helpers ────────────────────────────────────────────────────

function rpcOk(structuredContent: unknown): { code: number; stdout: string; stderr: string } {
  return {
    code: 0,
...
function rpcCurlError(): { code: number; stdout: string; stderr: string } {
  return { code: 7, stdout: "", stderr: "Failed to connect" };
}

/**
 * Build a scripted ExecFn that returns each result in order, distinguishing
 * the file_reservation_paths and release_file_reservations channels so a test
 * that triggers a release between reservation attempts doesn't desync.
 */
interface ScriptedExecOptions {
  reservation: Array<{ code: number; stdout: string; stderr: string }>;
  release?: Array<{ code: number; stdout: string; stderr: string }>;
}

function scriptedExec(opts: ScriptedExecOptions): {
  exec: ExecFn;
  ...
  const exec: ExecFn = vi.fn(async (_cmd: string, args: readonly string[]) => {
    ...
    if (toolName === "file_reservation_paths") {
      ...
      if (!next) throw new Error("scriptedExec: ran out of file_reservation_paths responses");
      return next;
    }
...
function makeOpts(exec: ExecFn, overrides: Partial<ReserveOrFailOpts> = {}): ReserveOrFailOpts {
  return {
    exec,
    cwd: "/test/cwd",
    agentName: "TestAgent",
...
describe("releaseReservations", () => {
  function releaseOpts(exec: ExecFn): ReleaseReservationsOpts {
    return { exec, cwd: "/test/cwd", agentName: "TestAgent" };
  }

  it("returns ok released:0 for empty id list without calling RPC", async () => {
    const exec = vi.fn();
    const result = await releaseReservations([], releaseOpts(exec as unknown as ExecFn));
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.released).toBe(0);
  ...
  it("returns ok with the server-reported released count on success", async () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.released).toBe(3);
  });
  ...
  it("falls back to id count when server omits the released field", async () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.released).toBe(2);
  });
  ...
  it("returns ok:false on RPC failure", async () => {
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.error.kind).toMatch(/network|timeout/);
  });
});
