...
function writeCheckpointWithState(dir: string, state: FlywheelState): void {
...
describe('doctor.outcome_rubric_validity', () => {
  ...
  function findRubricCheck(checks: Array<{ name: string }>): {
    name: string;
    severity: string;
    message: string;
    hint?: string;
    ...
    return row as { name: string; severity: string; message: string; hint?: string };
  }
