...
import { PostmortemDraftSchema, type ErrorCodeTelemetry } from '../types.js';
...
const fetchInboxResponses: Record<string, any> = {};
...
function gitCatFileFails(sha: string): ExecScript {
  return {
    match: (cmd, args) => cmd === 'git' && args[0] === 'cat-file' && args[1] === '-e' && args[2] === sha,
...
function gitMergeBase(result: { ok: boolean; sha?: string }): ExecScript {
  return {
...
function gitLogCommits(range: string | null, subjects: Array<{ sha: string; subject: string; author?: string }>): ExecScript {
  return {
    match: (cmd, args) => {
      if (cmd !== 'git' || args[0] !== 'log') return false;
      if (args[args.length - 1] !== '--no-merges') return false;
      if (range && args[1] !== range) return false;
...
describe('draftPostmortem', () => {
  ...
  it('P-3: draftPostmortem never invokes cm / flywheel_memory store — zero write calls', async () => {
    ...
    const storeCalls = calls.filter(
      (c) => c.cmd === 'cm' && (c.args[0] === 'add' || c.args[0] === 'store'),
    );
  ...
  it('P-4: sessionStartSha AND merge-base both fail → falls back to HEAD~10..HEAD, warnings present', async () => {
    ...
    expect(draft.warnings).toContain('postmortem_checkpoint_stale');
    expect(draft.hasWarnings).toBe(true);
    const usedHeadFallback = calls.some(
      (c) => c.cmd === 'git' && c.args[0] === 'log' && c.args.includes('HEAD~10..HEAD'),
    );
  ...
  it('top error codes: telemetry with 7 codes → draft includes top-5 sorted by count', async () => {
    const telemetry: ErrorCodeTelemetry = {
      version: 1,
