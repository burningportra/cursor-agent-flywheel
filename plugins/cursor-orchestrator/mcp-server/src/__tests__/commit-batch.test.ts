...
describe("resolveCommitBatchThreshold", () => {
  it("prefers checkpoint state over env and config", () => {
    ...
    try {
      expect(
        resolveCommitBatchThreshold("/repo", baseState({ commitBatchThreshold: 5 })),
      ).toBe(5);
    } finally {
      if (prev === undefined) delete process.env.FW_COMMIT_BATCH_THRESHOLD;
      else process.env.FW_COMMIT_BATCH_THRESHOLD = prev;
    }
  });
  ...
  it("returns 0 when nothing is configured", () => {
    ...
    try {
      ...
      if (prev !== undefined) process.env.FW_COMMIT_BATCH_THRESHOLD = prev;
    }
  });
});
...
describe("ensureBatchReviewBaseline", () => {
  it("seeds lastBatchReviewSha from cycleStartSha when missing", () => {
    const next = ensureBatchReviewBaseline(
      baseState({ cycleStartSha: "abc123" }),
      "deadbeef",
    );
