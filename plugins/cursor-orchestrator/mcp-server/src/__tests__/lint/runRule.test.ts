...
import type { Document, Finding, Rule, RuleContext } from "../../lint/types.js";

const doc: Document = { source: "", filePath: "x.md" };
const ctx: RuleContext = { filePath: "x.md", source: "" };

function makeFinding(ruleId: string, message: string): Finding {
...
describe("runRule", () => {
  ...
  it("normalizes a null/undefined return to empty findings", async () => {
    const rule: Rule = {
      ...
      description: "",
      severity: "error",
      check: () => undefined as unknown as Finding[],
    };
    const r = await runRule(rule, doc, ctx);
    expect(r.findings).toEqual([]);
    expect(r.internalError).toBeUndefined();
