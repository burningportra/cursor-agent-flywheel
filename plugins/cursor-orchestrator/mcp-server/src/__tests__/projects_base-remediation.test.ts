...
import {
  remediateProjectsBase,
  projectsBaseMisconfigHandler,
} from '../tools/remediations/projects_base_misconfig.js';
import { REMEDIATION_REGISTRY } from '../tools/remediate.js';
import type { HandlerCtx } from '../tools/remediate.js';
import type { ExecFn } from '../exec.js';

function makeTmp(): { tmp: string; cleanup: () => void } {
  const tmp = mkdtempSync(join(tmpdir(), 't61-'));
  return { tmp, cleanup: () => rmSync(tmp, { recursive: true, force: true }) };
}

function mockExec(ntmBase: string | null): ExecFn {
  return async (cmd: string, args: ReadonlyArray<string>) => {
    if (cmd === 'ntm' && args[0] === 'config' && args[1] === 'show') {
      if (ntmBase === null) return { code: 1, stdout: '', stderr: 'ntm: not found' };
...
describe('projectsBaseMisconfigHandler (T6.1) — doctor integration', () => {
  it('is registered in REMEDIATION_REGISTRY under projects_base_misconfig', () => {
    ...
    expect(REMEDIATION_REGISTRY['projects_base_misconfig']).toBe(
      projectsBaseMisconfigHandler,
    );
  });
