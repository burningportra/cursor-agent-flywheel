...
function makeCtx(cwd: string, stateOverrides: Partial<FlywheelState> = {}): { ctx: ToolContext; state: FlywheelState } {
  const state = makeBaseState(stateOverrides);
  const exec: ToolContext['exec'] = async () => ({ code: 1, stdout: '', stderr: 'not mocked' });
...
describe('flywheel_synthesize_rubric tool wrapper', () => {
  ...
  it('rubric_synthesized vs rubric_preserved discriminator: cache-hit path returns version 1 envelope', async () => {
    ...
    const planSha = await import('node:crypto').then((m) =>
      m.createHash('sha256').update(SAMPLE_PLAN).digest('hex'),
    );
    writeFileSync(
      join(dir, '.pi-flywheel/plans/plan/.rubric.lock'),
      JSON.stringify({ planContentSha: planSha, generatedAt: '2026-05-08T12:30:00Z', source: 'auto' }, null, 2),
      'utf8',
    );
  ...
  it('rubric_preserved: edited rubric on disk returns kind=preserved without spawning synth', async () => {
    ...
    mkdirSync(join(dir, '.pi-flywheel/plans/plan'), { recursive: true });
    writeFileSync(
      join(dir, '.pi-flywheel/plans/plan/rubric.md'),
      SAMPLE_RUBRIC.replace('source: auto', 'source: edited'),
      'utf8',
    );
    const { ctx } = makeCtx(dir, { planDocument: 'plan.md' });
    const result = await runSynthesizeRubric(ctx, { cwd: dir, action: 'synthesize' });
  ...
  it('Zod input failure surfaces invalid_input', async () => {
    const { ctx } = makeCtx(dir);
    // missing cwd
    const result = await runSynthesizeRubric(ctx, { action: 'synthesize' } as never);
    expect(result.isError).toBe(true);
    const sc = result.structuredContent as any;
    expect(sc.data.error.code).toBe('invalid_input');
  });
