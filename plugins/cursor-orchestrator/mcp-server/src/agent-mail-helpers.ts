...
import { agentMailRPC, type AgentMailReservation } from "./agent-mail.js";
...
interface ReservationResponse {
  granted?: AgentMailReservation[] | null;
  conflicts?: ReservationConflict[] | null;
}

interface AttemptOutcome {
  kind: "ok" | "conflict" | "rpc_error" | "empty";
  granted: AgentMailReservation[];
  conflicts: ReservationConflict[];
  rpcError?: { kind: string; message: string };
}

function extractIds(granted: AgentMailReservation[]): Array<number | string> {
...
async function attemptReserve(
  paths: string[],
  opts: ReserveOrFailOpts,
): Promise<AttemptOutcome> {
  const result = await agentMailRPC<ReservationResponse>(opts.exec, "file_reservation_paths", {
 ...
 */
export async function reserveOrFail(
  paths: string[],
  opts: ReserveOrFailOpts,
): Promise<ReserveOrFailResult> {
...
export async function releaseReservations(
  ...
  const result = await agentMailRPC<{ released?: number }>(opts.exec, "release_file_reservations", {
    project_key: opts.cwd,
    agent_name: opts.agentName,
  ...
  const releasedRaw = (result.data as { released?: unknown } | null)?.released;
