...
export const SolutionDocSchema = z.object({
  ...
  path: z.string().regex(
    /^docs\/solutions\/[a-z0-9-]+\/[a-z0-9-]+-\d{4}-\d{2}-\d{2}\.md$/,
    'path must match docs/solutions/<category>/<slug>-YYYY-MM-DD.md',
  ),
 ...
 */
export function slugifySolutionTitle(input: string): string {
  const s = input
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
 ...
 * call from tests.
 */
export function inferSolutionCategory(
  goal: string,
  touchedPaths: readonly string[],
): SolutionCategory {
  const hay = `${goal} ${touchedPaths.join(' ')}`.toLowerCase();
  if (/\btest\b|vitest|jest|spec\b|\bflak/i.test(hay)) return 'test';
  if (/\bbuild\b|tsc|webpack|vite|rollup|compile/i.test(hay)) return 'build';
  if (/\bskill\b|\breadme\b|\bdocs?\b|changelog/i.test(hay)) return 'docs';
  if (/agent-mail|swarm|worktree|coordinat/i.test(hay)) return 'coordination';
  if (/refactor|rename|extract|split|migrate/i.test(hay)) return 'refactor';
  if (/\bcli\b|tool|editor|ergonom/i.test(hay)) return 'tooling';
  if (/runtime|produc|crash|panic|oom/i.test(hay)) return 'runtime';
  return 'general';
}
 ...
 */
export function renderSolutionDoc(doc: SolutionDoc): string {
