...
const PROFILE_INTENT_STALE_GREEN_HINT =
  'No action needed; continue the flywheel.';
const PROFILE_INTENT_STALE_INACTIVE_HINT =
  'No intent-file watch registered; staleness is tracked after flywheel_profile or plan registration.';
const PROFILE_INTENT_STALE_YELLOW_HINT =
  'Call flywheel_profile({ force: true }) to refresh the repo profile and re-register intent file hashes after plan/rubric edits.';
...
async function checkProfileIntentStale(
  cwd: string,
  signal: AbortSignal,
  now: () => number,
): Promise<DoctorCheck> {
  const start = now();
  if (signal.aborted) return abortedCheck('profile_intent_stale');
