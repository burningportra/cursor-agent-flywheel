...
export const ROBOT_DOCS_VERSION = 1 as const;
...
const SECTIONS: Record<RobotDocsSection, DocSection> = {
  getting_started: {
    key: 'getting_started',
...
function pickSection(raw: unknown): RobotDocsSection | 'all' {
  if (typeof raw !== 'string') return 'all';
  if (raw === 'all') return 'all';
  if ((ROBOT_DOCS_SECTIONS as readonly string[]).includes(raw)) {
    return raw as RobotDocsSection;
  }
  return 'all';
}

export function buildRobotDocs(section: RobotDocsSection | 'all'): RobotDocsEnvelope {
  const selected = section === 'all'
    ? (ROBOT_DOCS_SECTIONS as readonly RobotDocsSection[]).map((k) => SECTIONS[k])
    : [SECTIONS[section]];
