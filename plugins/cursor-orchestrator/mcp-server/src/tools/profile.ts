...
import { loadFlywheelConfig, type FlywheelConfigProfile } from '../flywheel-config.js';
import {
 ...
 */
export function scheduleProfileAutoRefresh(ctx: ToolContext, config?: FlywheelConfigProfile): void {
  if (!shouldScheduleProfileAutoRefresh(ctx.state, config)) return;
  ...
  void (async () => {
    try {
      ...
      Object.assign(
        state,
        registerProfileWatch(
          clearProfileStaleFlags({ ...state, repoProfile: profile }),
          cwd,
