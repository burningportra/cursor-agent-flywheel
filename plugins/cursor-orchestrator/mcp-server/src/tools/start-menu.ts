...
import { buildStartMenu, inferStartMenuVariant, type StartMenuVariant } from "../cursor-start-menu.js";
...
export async function runStartMenu(
  ...
  return makeOkToolResult(
    "flywheel_start_menu",
    "start_menu",
    `flywheel_start_menu: variant=${menu.variant} | AskQuestion(data.askQuestion) → routeHints`,
    data,
  );
}
