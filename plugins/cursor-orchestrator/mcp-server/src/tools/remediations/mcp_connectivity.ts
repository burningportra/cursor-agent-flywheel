...
function distServerExists(cwd: string): boolean {
...
export const mcpConnectivityHandler: RemediationHandler = {
  ...
  async buildPlan(ctx: HandlerCtx) {
    ...
    return {
      description:
        'mcp-server/dist/server.js missing — run `npm ci && npm run build` inside mcp-server/.',
  ...
  async execute(ctx: HandlerCtx) {
    ...
    if (build.code !== 0) log.warn('npm run build exited non-zero', { exitCode: build.code });
    return {
      stepsRun: 2,
      stdout: `${ci.stdout}\n${build.stdout}`,
      stderr: `${ci.stderr}\n${build.stderr}`,
  ...
  async verifyProbe(ctx: HandlerCtx) {
    const present = distServerExists(ctx.cwd);
    if (!present) log.warn('verifyProbe: dist/server.js still missing after remediation');
    return present;
  },
};
