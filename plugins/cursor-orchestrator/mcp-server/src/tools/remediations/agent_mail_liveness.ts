...
function formatStep({ label, code, stdout, stderr }: StepOutput): string {
...
async function runRequiredStep(
  ...
  const output: StepOutput = { label, code: res.code, stdout: res.stdout, stderr: res.stderr };
  if (res.code !== 0) {
...
export const agentMailLivenessHandler: RemediationHandler = {
  ...
  async buildPlan(_ctx: HandlerCtx) {
    return {
      description:
        'Service-aware Agent Mail fix: stop the supervised runtime to release mailbox activity locks, run `am doctor repair --yes`, run `am doctor archive-normalize --yes`, restart Agent Mail, then verify /health/liveness.',
      steps: [
  ...
  async execute(ctx: HandlerCtx) {
    ...
    outputs.push(
      await runRequiredStep(ctx, 'command -v am', 'bash', [
        '-lc',
        'command -v am >/dev/null 2>&1',
      ]),
    );
    outputs.push(
      await runBestEffortStep(ctx, 'stop Agent Mail service/runtime', 'bash', [
        '-lc',
        STOP_SERVICE_SCRIPT,
      ]),
    );
    ...
    try {
      outputs.push(
        await runRequiredStep(ctx, 'am doctor repair --yes', 'am', ['doctor', 'repair', '--yes']),
      );
      outputs.push(
        await runBestEffortStep(ctx, 'am doctor archive-normalize --yes', 'am', [
          'doctor',
          'archive-normalize',
          '--yes',
        ]),
      ...
      outputs.push(
        await runBestEffortStep(ctx, 'restart Agent Mail service/runtime', 'bash', [
          '-lc',
