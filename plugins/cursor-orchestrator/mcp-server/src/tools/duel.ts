...
export interface DuelToolOutcome {
  ...
  ntmFallback?: { duelCommand: string; reason: string };
}

const VALID_MODES: DuelMode[] = [
  'ideas',
  'architecture',
  'security',
  'reliability',
...
export async function runDuel(
  ...
  const mode: DuelMode =
    args.mode && VALID_MODES.includes(args.mode as DuelMode)
      ? (args.mode as DuelMode)
      : 'ideas';
  ...
  if (!models.wizard_a?.trim() || !models.wizard_b?.trim()) {
    return makeToolError(
      ...
      state.phase,
      'invalid_input',
      'wizard_a and wizard_b must be non-empty Cursor model slugs.',
    );
  }
