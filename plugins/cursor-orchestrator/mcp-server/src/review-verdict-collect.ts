...
import type { BatchReviewVerdict, McpToolResult, ToolContext } from './types.js';
...
function needsAttentionFallback(
  ...
  const rawSnippet =
    rawVerdict.length > 2048 ? `${rawVerdict.slice(0, 2048)}\n…(truncated)` : rawVerdict;
  ...
  return okCollect(
    state.phase,
    `## Review: NEEDS ATTENTION (fallback) — ${opts.provenanceKey}\n\n` +
      `${reason}\n\n` +
...
export async function collectReviewVerdict(
  ctx: ToolContext,
  opts: CollectReviewVerdictOpts,
  ...
  if (opts.rawVerdict !== undefined) {
    ...
    try {
      ...
      return okCollect(
        state.phase,
        `Verdict file not readable at \`${opts.verdictPath}\`: ${errMsg(err)}`,
        {
          kind: opts.kind,
  ...
  try {
    ...
    try {
      appendMemory(
        cwd,
        `malformed review verdict (${opts.provenanceKey}): ${errMsg(err)}`,
        opts.memoryTag,
      );
    } catch { /* best-effort */ }
  ...
  if (!parseResult.success) {
    ...
    try {
      appendMemory(
        cwd,
        `malformed review verdict schema (${opts.provenanceKey}): ${issues}`,
        opts.memoryTag,
      );
    } catch { /* best-effort */ }
    return needsAttentionFallback(
      ...
      opts,
      rawVerdict,
      `Verdict schema validation failed: ${issues}`,
    );
  }
  ...
  if (verdict.sha_range !== opts.expectedShaRange) {
    return needsAttentionFallback(
      ...
      opts,
      rawVerdict,
      `Verdict sha_range "${verdict.sha_range}" does not match expected "${opts.expectedShaRange}".`,
    );
  }
  ...
  if (verdict.status === 'pass') {
    const passNext =
      opts.kind === 'hit_me_review_verdict'
        ? { kind: 'proceed_looks_good' as const }
        : { kind: 'advance_wave' as const };
  ...
  try {
    synthesizedIds = await synthesizeBeadsFromFindings(
      cwd,
      state,
      verdict.findings,
      opts.provenanceKey,
    ...
    try {
      appendMemory(
        cwd,
        `review synthesize failure (${opts.provenanceKey}): ${synthesisError}`,
        opts.memoryTag,
      );
    } catch { /* best-effort */ }
  }
  ...
  if (synthesisError) {
    return okCollect(
      state.phase,
      `${opts.blockingMessagePrefix} (synthesis failed)\n\n` +
