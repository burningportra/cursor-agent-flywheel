...
export function verifyPolicyFromConfig(
  verify?: FlywheelConfigVerify,
): VerifyPolicy {
  if (!verify) return { ...DEFAULT_VERIFY_POLICY };
  return {
    testCwd:
      typeof verify.test_cwd === 'string' && verify.test_cwd.trim()
        ? verify.test_cwd.trim()
        : DEFAULT_VERIFY_POLICY.testCwd,
    buildSlot:
      typeof verify.build_slot === 'string' && verify.build_slot.trim()
        ? verify.build_slot.trim()
        : DEFAULT_VERIFY_POLICY.buildSlot,
    maxWorkers:
      typeof verify.max_workers === 'number' && verify.max_workers >= 1
        ? Math.floor(verify.max_workers)
        : DEFAULT_VERIFY_POLICY.maxWorkers,
    allowFullSuiteWhenAlone:
      typeof verify.allow_full_suite_when_alone === 'boolean'
...
export function formatScopedTestCommand(
...
export function formatFullSuiteTestCommand(
  policy: VerifyPolicy = DEFAULT_VERIFY_POLICY,
): string {
  return `cd ${policy.testCwd} && npm test`;
}

export function formatVerifyForbiddenSummary(
  policy: VerifyPolicy = DEFAULT_VERIFY_POLICY,
): string {
  return [
...
export function formatImplAgentVerifyLines(
...
export function formatSwarmVerifyBlock(
  policy: VerifyPolicy = DEFAULT_VERIFY_POLICY,
): string {
  return [
