...
When `metadata.pluginRoot` is `"plugins"`, `source` is relative to that directory (e.g. `./my-new-plugin` → `plugins/my-new-plugin/`). If you omit `pluginRoot`, use a path from the repository root instead.
