...
>
> **NTM optional.** Only when the user explicitly wants tmux panes, set `FW_IMPL_BACKEND=ntm` or follow the NTM appendix at the bottom. Do not silently fall back to NTM.
...
When `FW_IMPL_BACKEND=ntm` or the user insists on tmux panes: invoke `/vibing-with-ntm`, resolve `NTM_PROJECT`, spawn via `ntm spawn` + `ntm --robot-send` with `program: "claude-code"` per pane. See the upstream NTM-first doc in git history if you need the full pane ladder.
